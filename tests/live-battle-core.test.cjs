...
test('實際 AQ／RT 欄位拆出五項、商品與影音漏搭，並遮罩案件識別', () => {
  ...
  const rt = Core.parseMatrix([
    ['案件類型', '店點', '門號', '變更資費', '商品型號', '前台服務人員', '客戶分類', '合約編號', '專案代號', '合約代碼'],
    ['RT續約', '通化', '0912345678', '5G 1399', 'Pixel 11 Pro', 'DNB10146_5514709 王克業', '一般戶', '主約', '提前續約', 'VK101'],
    ['RT續約', '通化', '0912345678', '5G 1399', '', 'DNB10146_5514709 王克業', '一般戶', 'KKBOX 3個月', '搭贈', 'GIFT01'],
    ['RT續約', '酒泉', '0923456789', '5G 599', 'Galaxy A57', '李小華', '企客', '主約', '一般續約', 'VK102']
  ], { kind: 'rt', fileName: 'RT.csv', stores: sourceStores });
  ...
  assert.equal(rt.giftAudit[0].contractCode, 'VK101');
});

test('影音漏搭先篩 VK 合約代碼，只認明確 5G 599 型含以上並排除企客與 4G 案', () => {
  const rt = Core.parseMatrix([
    ['案件類型', '店點', '門號', '變更資費', '商品型號', '前台服務人員', '客戶分類', '合約編號', '專案代號', '合約代碼'],
    ['RT續約', '通化', '0900000001', '5G 599', '一般商品', '王一', '一般戶', '主約', '一般續約', 'VK201'],
    ['RT續約', '通化', '0900000001', '5G 599', '', '王一', '一般戶', '重複明細', '一般續約', 'VK201'],
    ['RT續約', '酒泉', '0900000002', '5G 1399', '一般商品', '李二', '一般戶', '主約', '提前續約', 'VK202'],
    ['RT續約', '大稻埕', '0900000003', '4G 799', 'Pixel 11 Pro (5G)', '張三', '一般戶', '主約', '一般續約', 'VK203'],
    ['RT續約', '萬大', '0900000004', '4G 599', 'Galaxy A57 (5G)', '林四', '一般戶', '主約', '一般續約', 'VK204'],
    ['RT續約', '六張犁', '0900000005', '599', '一般商品', '陳五', '一般戶', '主約', '一般續約', 'VK205'],
    ['RT續約', '復興南', '0900000006', '5G 599', '一般商品', '周六', '企客', '主約', '一般續約', 'VK206'],
    ['RT續約', '杭州南', '0900000007', '4G升5G 999', '一般商品', '吳七', '一般戶', '主約', '一般續約', 'VK207'],
    ['RT續約', '永吉', '0900000008', '5G 999', '一般商品', '鄭八', '一般戶', '主約', '一般續約', 'VL999']
  ], { kind: 'rt', fileName: 'RT.csv', stores: sourceStores });
  ...
  assert.ok(!rt.giftAudit.some(item => ['大稻埕', '萬大', '六張犁', '復興南', '永吉'].includes(item.store)));
  assert.equal(rt.giftAudit.filter(item => item.caseId === '090***001').length, 1);
});

test('影音漏搭以門號優先去重，不因同門號多個合約編號重複列入', () => {
  const rt = Core.parseMatrix([
    ['店點', '合約編號', '門號', '變更資費', '前台服務人員', '客戶分類', '合約代碼'],
    ['酒泉', 'CONTRACT-01', '0911222333', '5G 999', '李小華', '一般戶', 'VK301'],
    ['酒泉', 'CONTRACT-02', '0911222333', '5G 999', '李小華', '一般戶', 'VK302']
  ], { kind: 'rt', fileName: 'RT.csv', stores: sourceStores });

  assert.equal(rt.giftAudit.length, 1);
  assert.equal(rt.giftAudit[0].caseId, '091***333');
  assert.equal(rt.giftAudit[0].contractCode, 'VK301／VK302');
});
