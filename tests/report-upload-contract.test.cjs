...
test('同源 HtmlService 僅使用 google.script.run，台獎預覽採分段上傳與輪詢', () => {
  ...
  assert.match(code, /deploymentVersion: '39'/);
  assert.match(code, /buildCommit: '[0-9a-f]{40}'/);
  ...
  assert.match(htmlPage, /schemaVersion: 'phone-awards-v1'/);
  assert.match(htmlPage, /publishBlockingReasons/);
  assert.match(htmlPage, /awardAmounts:/);
  assert.match(htmlPage, /achievementRates:/);
  assert.match(htmlPage, /models:/);
