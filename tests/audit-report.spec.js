...
async function mockApi(page,options={}){
  ...
  await page.route('https://script.google.com/**',async route=>{
    ...
    else if(payload.action==='audit_submit_auth')body=payload.employeeId==='EMP001'&&/^[A-Za-z0-9_-]{16,128}$/.test(payload.deviceId||'')?{status:'ok',token:`store-token-${payload.submission_id}`,expiresIn:1800,profile:{store_id:'DNB10062',store_name:'台北酒泉',inspector_name:'測＊員'}}:{status:'error',message:'unauthorized'};
    ...
    else if(payload.action==='audit_start'){currentStatus=ownStatus({submission_id:payload.submission_id,submission_status:'draft',submitted_at:'',items:ITEMS.map(item=>({...item,status:'draft',reviewer_comment:'',note:'',photo_count:0,photos:[]}))});body=currentStatus;}
...
test('nine canonical stores, required fields, multi-add, delete, preview and ten-photo limit',async({page})=>{
  ...
  await expect(page.locator('#storeSelect')).toBeDisabled();await expect(page.locator('#inspectorName')).toHaveAttribute('readonly','');await expect(page.locator('#submitButton')).toBeDisabled();await expect(page.locator('#missingText')).toContainText('門市店點');
  await authorizeStore(page);await expect(page.locator('#storeAuthMessage')).toContainText('核准裝置驗證成功');await expect(page.locator('#storeSelect')).toHaveValue('DNB10062');await expect(page.locator('#inspectorName')).toHaveValue('測＊員');await addPhoto(page,0,'one.png');await addPhoto(page,0,'two.png');await expect(page.locator('.audit-item').first().locator('.photo-tile')).toHaveCount(2);
...
test('partial upload failure preserves successes and retries only the failed photo before readback success',async({page})=>{
  const mock=await mockApi(page,{failOnce:true});await page.goto(PAGE_URL);await authorizeStore(page);await addPhoto(page,0,'ok.png');await addPhoto(page,0,'fail.png');await addPhoto(page,1,'op.png');await addPhoto(page,2,'counter.png');
...
test('supervisor overview stays locked until PT auth and expired token reauth preserves store draft',async({page})=>{
  const mock=await mockApi(page,{overviewExpired:true});await page.goto(PAGE_URL);await authorizeStore(page);await addPhoto(page,0,'draft.png');await page.locator('#modeSwitch').click();await expect(page.locator('#supervisorGate')).toBeVisible();expect(mock.calls.filter(call=>call.action==='audit_overview')).toHaveLength(0);
  ...
  await page.locator('#modeSwitch').click();await expect(page.locator('#inspectorName')).toHaveValue('測＊員');await expect(page.locator('.audit-item').first().locator('.photo-tile')).toHaveCount(1);
});

test('unapproved or expired Approved Device authorization is rejected without losing the local draft',async({page})=>{
  const mock=await mockApi(page,{expireStoreToken:true});await page.goto(PAGE_URL);await addPhoto(page,0,'draft-kept.png');
  ...
  await authorizeStore(page);await expect(page.locator('#storeAuthMessage')).toContainText('已過期');await expect(page.locator('#inspectorName')).toHaveValue('測＊員');await expect(page.locator('.audit-item').first().locator('.photo-tile')).toHaveCount(1);
...
test('single returned item is the only unlocked upload target and keeps reason/original photos',async({page})=>{
  ...
  await page.addInitScript(({draft,session})=>{localStorage.setItem('bei12b_audit_draft_v1',JSON.stringify(draft));sessionStorage.setItem('bei12b_audit_store_session',JSON.stringify(session));sessionStorage.setItem('north12b_private_dashboard_employee_id','EMP001');},{draft:{batch_id:'audit-cleaning-202608',store_id:'DNB10062',inspector_name:'測＊員',submission_id:'submission_test_123456789012345',edit_token:'edit_test_123456789012345678901234',notes:{},items:{}},session:{token:'store-token-submission_test_123456789012345',auth_source:'approved-device',batch_id:'audit-cleaning-202608',store_id:'DNB10062',submission_id:'submission_test_123456789012345'}});
