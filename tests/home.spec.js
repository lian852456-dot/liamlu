...
test('同仁大廳以 KPI、台獎戰情為前兩個入口，既有店務檢查入口保持安全連結', async ({ page }) => {
  ...
  await expect(staffCards.nth(5)).toContainText('門市使用');
  await expect(staffCards.nth(5)).toContainText('檢');
  await expect(staffCards.nth(5)).toContainText('門市店務檢查');
  await expect(staffCards.nth(5)).toContainText('完成每日、半月與每月店務檢查，支援備註及照片／影片上傳。');
  await expect(staffCards.nth(5)).toHaveAttribute('href', STORE_INSPECTION_URL);
  await expect(staffCards.nth(5)).toHaveAttribute('target', '_blank');
  await expect(staffCards.nth(5)).toHaveAttribute('rel', 'noopener noreferrer');
...
test('同仁大廳手機版維持單欄且文字未水平溢出，既有入口連結不變', async ({ page }) => {
  ...
  expect(await page.locator(`.card:not([href="kpi-battle.html"]):not([href="awards-battle.html"]):not([href="${STORE_INSPECTION_URL}"])`).evaluateAll(
