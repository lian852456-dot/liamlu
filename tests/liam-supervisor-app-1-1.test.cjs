...
test('App recovery runtime keeps existing reads and disables half-month writes', () => {
  ...
  assert.match(code, /new Set\(\['sread','ptsummary','ptdetail','ptmileage2','ptvisit_read','hread'\]\)/);
...
test('PWA cache is versioned for App 1.2 and includes local icon library', () => {
  ...
  assert.match(worker, /liam-supervisor-app-1-2-patrol-isolated-recovery-20260916-v1/);
  ...
  assert.match(html, /app\.js\?v=patrol-isolated-recovery-20260916/);
