...
- 2026-09-11 第①張 PNG 格式修正已發布：AQ／RT 明細列文字由列底改為列中央，排除下一列底色覆蓋文字下半部；只動 PNG 顯示與快取版本，不改解析、數值或其他戰報。功能 commit `4660441`、Pages run `34593201234` 成功，正式頁已回讀新版快取與置中座標；待 Liam 以真實檔重新下載第①張做實機視覺確認。
- 2026-09-11 可讀性加強已發布：網頁與四張 PNG 統一優先使用 Microsoft YaHei 粗體並提高深色文字對比；九店資費／上線數與商品上線數另行放大，不改資料或計算。功能 commit `66d91b6`、Pages run `34594958938` 成功，正式 HTML／CSS／JS 已回讀新版快取、字型與字級。
...
- RT 漏搭以同一門號／案件的全部列判讀；先從「合約代碼」（相容促案／專案／優惠／服務／產品代碼）篩出 `VK…` 合約，缺少 VK 資格即不列入。同案件多筆 VK 明細只認列一次；VK 列的「變更／申辦資費」必須明確為 5G 599 型（含）以上，提前續約同樣適用，企客與 4G 案排除，KKBOX／MyVideo 任一缺少即列出遮罩明細與合約代碼。商品型號中的 5G 不得作為資格證據，單憑月租數字也不得推定為 5G。
