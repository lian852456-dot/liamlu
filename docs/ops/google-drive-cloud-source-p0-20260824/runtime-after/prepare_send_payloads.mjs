...
const phoneOutputDir = path.resolve(process.env.PHONE_AWARDS_OUTPUT_DIR || outputDir);
...
export function validateAwardSourceFreshness(summary, expectedRunDate) {
  ...
  for (const [kind, expectedBasename] of Object.entries(REQUIRED_AWARD_SOURCE_BASENAMES)) {
    ...
    result[kind] = {
      ...
      ...(entry.eTag ? { eTag: String(entry.eTag) } : {}),
      ...(provider === 'google-drive-cloud' ? { googleDriveFileId: String(entry.driveItemId) } : {}),
...
const phoneSummary = JSON.parse(await fs.readFile(path.join(phoneOutputDir, "phone_awards_update_summary.json"), "utf8"));
...
const phoneBodyPath = path.join(phoneOutputDir, `email_body_phone_awards_${reportDateIso}.txt`);
...
const phoneAttachments = [
  path.join(phoneOutputDir, "Y26重點台獎手機_台獎更新.xlsx"),
  path.join(phoneOutputDir, "01-08-03_手機競賽日報_店點達成率_台獎機款更新.xlsx"),
  path.join(phoneOutputDir, "01-08-04_手機競賽日報_個人達成率_個人台獎更新.xlsx"),
  path.join(phoneOutputDir, "台獎機款.png"),
  path.join(phoneOutputDir, "個人台獎.png"),
  path.join(phoneOutputDir, "台獎機款_店點進度點名.md"),
];
...
const manifestPath = path.resolve(
  process.env.REPORT_MANIFEST_PATH || path.join(logsDir, `run-manifest-${dateToken}.json`),
);
const preflightPath = path.resolve(
  process.env.REPORT_PREFLIGHT_PATH || path.join(logsDir, `preflight-${dateToken}.md`),
);
