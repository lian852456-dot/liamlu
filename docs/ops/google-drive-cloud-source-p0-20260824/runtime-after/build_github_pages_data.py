...
def validated_award_source_files(
    summary: dict[str, Any], report_run_date: str, data_cutoff_date: str
    ...
    batch = summary.get("source_batch")
    if not isinstance(batch, dict):
        raise RuntimeError("台獎同批來源紀錄缺少，禁止建立 snapshot")
    if (batch.get("schema_version") != "north12b-source-batch/v1" or
            str(batch.get("report_run_date") or "") != report_run_date or
            str(batch.get("data_cutoff_date") or "") != data_cutoff_date or
            not re.fullmatch(r"[a-f0-9]{64}", str(batch.get("batch_id") or ""))):
        raise RuntimeError("台獎同批來源紀錄與 KPI 日期不一致，禁止建立 snapshot")
    batch_kpi = batch.get("kpi")
    if (not isinstance(batch_kpi, dict) or
            not re.fullmatch(r"[a-f0-9]{64}", str(batch_kpi.get("sha256") or "")) or
            str(batch_kpi.get("source_data_date") or "") != data_cutoff_date or
            not isinstance(batch_kpi.get("date_provenance"), dict)):
        raise RuntimeError("KPI 同批來源 lineage 不完整，禁止建立 snapshot")
    ...
    for kind, expected_basename in REQUIRED_AWARD_SOURCE_BASENAMES.items():
        ...
        drive_item_id = str(entry.get("driveItemId") or "")
        modified = str(entry.get("lastModifiedDateTime") or "")
        etag = str(entry.get("eTag") or "")
        size = entry.get("size")
        source_data_date = str(entry.get("source_data_date") or "")
        source_date_range = str(entry.get("source_date_range") or "")
        staged_sha256 = str(entry.get("staged_sha256") or "").lower()
        if basename != expected_basename:
            raise RuntimeError(f"台獎 {kind} canonical basename 不符：{basename}")
        if not re.fullmatch(r"[a-f0-9]{64}", sha256):
            raise RuntimeError(f"台獎 {kind} SHA-256 缺少")
        if (provider not in {"onedrive-cloud", "google-drive-cloud"} or not drive_item_id or not modified or
                (provider == "onedrive-cloud" and not etag) or
                not isinstance(size, int) or size <= 0 or
                str(entry.get("run_id") or "") != run_id):
            raise RuntimeError(f"台獎 {kind} cloud identity/run_id 缺少")
        if source_data_date != data_cutoff_date:
            raise RuntimeError(f"台獎 {kind} 資料截止日與 KPI 不一致，禁止建立 snapshot")
        provenance = entry.get("date_provenance")
        if (not source_date_range or not isinstance(provenance, dict) or
                provenance.get("value_kind") != "literal-string" or
                not isinstance(provenance.get("cells"), list) or
                not provenance["cells"] or
                any(not isinstance(cell, dict) or
                    cell.get("formula") is not None or
                    cell.get("formula_layer_value") != source_date_range or
                    cell.get("cached_value") != source_date_range or
                    cell.get("display_value") != source_date_range
                    for cell in provenance["cells"])):
            raise RuntimeError(f"台獎 {kind} Excel 日期 lineage 不完整，禁止建立 snapshot")
        if staged_sha256 != sha256:
            raise RuntimeError(f"台獎 {kind} 原始與 staged SHA-256 不一致，禁止建立 snapshot")
        batch_entry = (batch.get("awards") or {}).get(kind)
        if (not isinstance(batch_entry, dict) or
                str(batch_entry.get("sha256") or "").lower() != sha256 or
                str(batch_entry.get("source_data_date") or "") != data_cutoff_date or
                batch_entry.get("date_provenance") != provenance):
            raise RuntimeError(f"台獎 {kind} 同批來源 hash 不一致，禁止建立 snapshot")
        source_files[kind] = {
            "basename": basename,
        ...
        if etag:
            source_files[kind]["eTag"] = etag
        if provider == "google-drive-cloud":
            source_files[kind]["googleDriveFileId"] = drive_item_id
    return source_files
...
def extract_phone_awards(
    ...
    source_files = validated_award_source_files(summary, report_run_date, data_cutoff_date)
...
def extract_kpi_battle(
    ...
    # New store-personal workbook layouts may omit role labels that are present
    # in the immediately previous formal source. Pass the previous rows into the
    # canonical extractor before applying DOD so the current 40 people are not
    # silently dropped.
    personal_date_range, personal_rows = extract_rows(
        source_path,
        previous_rows=previous_personal_rows,
    )
    if previous_personal_rows:
...
def main() -> None:
    ...
    parser.add_argument("--kpi-only", action="store_true", help="Build only the KPI supplement; never read or write awards outputs")
    parser.add_argument("--awards-only", action="store_true", help="Build only awardsBattle; never read or write the KPI component")
    parser.add_argument("--kpi-run-id", help="Protected kpicalc processingRunId required by --kpi-only")
    parser.add_argument("--awards-summary-path", type=Path, help="Fresh run-scoped phone_awards_update_summary.json")
    args = parser.parse_args()

    if args.kpi_only and args.awards_only:
        raise RuntimeError("--kpi-only 與 --awards-only 不可同時使用")
    ...
    if args.kpi_only:
        if not args.kpi_run_id:
            raise RuntimeError("--kpi-only 必須提供 protected KPI --kpi-run-id")
        ...
        print(json.dumps({
            ...
            "stores": len(kpi_battle.get("stores", [])),
            "personal": len(kpi_battle.get("personal", [])),
            "private_kpi_output": str(args.private_kpi_output),
            "awards_written": False,
        }, ensure_ascii=False))
        return

    if args.awards_only:
        summary_path = args.awards_summary_path or (args.outputs_dir / "phone_awards_update_summary.json")
        if not summary_path.exists():
            raise RuntimeError(f"fresh awards summary 不存在：{summary_path}")
        summary = json.loads(summary_path.read_text(encoding="utf-8"))
        validated_award_source_files(summary, report_run_date_iso, data_cutoff_date_iso)
        args.private_kpi_output.parent.mkdir(parents=True, exist_ok=True)
        awards_output = args.private_kpi_output.parent / "phone-awards-battle-latest.json"
        phone_battle = extract_phone_awards_battle(
            summary_path,
            report_run_date_iso,
            data_cutoff_date_iso,
        )
        awards_output.write_text(
        ...
        print(json.dumps({
            ...
            "phone_items": phone_battle.get("phone_items"),
            "store_rows": phone_battle.get("store_rows"),
            "stores": len(phone_battle.get("stores", [])),
            "private_awards_output": str(awards_output),
            "kpi_written": False,
        }, ensure_ascii=False))
        return

    # Validate the shared source batch before writing even a local website
    # artifact.  A KPI-only preview must never be produced beside stale awards.
    summary_path = args.outputs_dir / "phone_awards_update_summary.json"
    if not summary_path.exists():
        raise RuntimeError("phone_awards_update_summary.json 不存在，禁止建立 KPI／台獎網站資料")
    batch_summary = json.loads(summary_path.read_text(encoding="utf-8"))
    validated_award_source_files(batch_summary, report_run_date_iso, data_cutoff_date_iso)
