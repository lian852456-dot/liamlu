...
import { loadOneDriveGraphAccessToken } from "./preflight_onedrive_cloud_sources.mjs";
...
const sourceMode = String(process.env.REPORT_SOURCE_MODE ?? "onedrive-cloud").trim();
...
if (sourceMode === "onedrive-cloud") {
  ...
  cloudManifest = await resolveOneDriveCloudSourceSet({
    graphClient: new OneDriveGraphClient({ accessToken: await loadOneDriveGraphAccessToken() }),
  ...
  throw new Error(
    "production KPI source mode must be onedrive-cloud; local fallback requires "
...
console.log(JSON.stringify({
  ...
  source_provider: sourceMode === "onedrive-cloud" ? "onedrive-cloud" : "local-emergency",
