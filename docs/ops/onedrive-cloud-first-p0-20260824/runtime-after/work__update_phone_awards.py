...
def kpi_identity_for_batch(report_run_date):
    ...
    if direct_source.is_file():
        ...
        cloud_identity = (
            ...
            if PHONE_AWARDS_SOURCE_MODE == "onedrive-cloud" else None
        )
...
def validate_same_source_batch(freshness, store_source, person_source):
    ...
    for kind, source in source_specs.items():
        ...
        staged = (
            ...
            if entry.get("provider") == "onedrive-cloud" and entry.get("staged_path")
...
def load_cloud_source_manifest():
    ...
    if not PHONE_AWARDS_CLOUD_MANIFEST_PATH:
        raise RuntimeError("OneDrive cloud source manifest is required; local fallback is OFF")
    manifest_path = Path(PHONE_AWARDS_CLOUD_MANIFEST_PATH).expanduser()
    if not manifest_path.is_file():
        raise FileNotFoundError(f"OneDrive cloud source manifest not found: {manifest_path}")
    payload = json.loads(manifest_path.read_text(encoding="utf-8"))
    if payload.get("provider") != "onedrive-cloud":
        raise RuntimeError("OneDrive cloud source manifest provider must be onedrive-cloud")
    if payload.get("schema_version") != "north12b-onedrive-cloud-source/v1":
        raise RuntimeError("OneDrive cloud source manifest schema is invalid")
    if str(payload.get("report_run_date") or "") != REPORT_RUN_DATE:
        raise RuntimeError("OneDrive cloud source manifest report_run_date mismatch")
    run_id = str(payload.get("run_id") or "").strip()
    if not run_id:
        raise RuntimeError("OneDrive cloud source manifest run_id is missing")
    if os.environ.get("REPORT_RUN_ID", "").strip() not in {"", run_id}:
        raise RuntimeError("OneDrive cloud source manifest run_id mismatch")
    if not isinstance(payload.get("sources"), dict):
        raise RuntimeError("OneDrive cloud source manifest sources are missing")
...
def cloud_source_identity(kind, source):
    ...
    if provider != "onedrive-cloud" or not drive_item_id or not modified or not etag:
        raise RuntimeError(f"{kind} OneDrive cloud version identity is incomplete")
    try:
        datetime.fromisoformat(modified.replace("Z", "+00:00"))
    except ValueError as exc:
        raise RuntimeError(f"{kind} OneDrive lastModifiedDateTime is invalid") from exc
    if not isinstance(size, int) or size <= 0:
        raise RuntimeError(f"{kind} OneDrive cloud size is invalid")
    if not re.fullmatch(r"[a-f0-9]{64}", digest) or staged_digest != digest:
        raise RuntimeError(f"{kind} OneDrive cloud/staged SHA-256 is invalid")
    if run_id != str(manifest.get("run_id") or ""):
        raise RuntimeError(f"{kind} OneDrive cloud run_id mismatch")
    if staged_path.resolve() != Path(source).resolve():
        raise RuntimeError(f"{kind} staged path does not match OneDrive cloud manifest")
    if not staged_path.is_file() or staged_path.stat().st_size != size:
        raise RuntimeError(f"{kind} staged file size does not match OneDrive cloud metadata")
    actual = file_sha256(staged_path)
    if actual != digest:
        raise RuntimeError(f"{kind} staged SHA-256 does not match OneDrive cloud download")
    return {
        "provider": provider,
...
def resolve_exact_or_fresh_source(env_path, prefix, kind, expected_data_date):
    ...
    if PHONE_AWARDS_SOURCE_MODE == "onedrive-cloud":
        if not env_path:
            raise RuntimeError(f"{kind} OneDrive cloud staged source is required; local fallback is OFF")
        ...
        return exact, "onedrive-cloud"
    if PHONE_AWARDS_SOURCE_MODE != "local-emergency" or not PHONE_AWARDS_LOCAL_EMERGENCY_ENABLED:
        raise RuntimeError(
            "production source mode must be onedrive-cloud; local emergency fallback requires "
...
def validate_fresh_award_sources(store_source, person_source, store_origin, person_origin):
    ...
    run_id = (
        ...
        if PHONE_AWARDS_SOURCE_MODE == "onedrive-cloud" else os.environ.get("REPORT_RUN_ID", "").strip()
    ...
    if PHONE_AWARDS_SOURCE_MODE == "onedrive-cloud" and previous:
        ...
        str(previous["store"].get("sha256") or "") == source_hashes["store"]
        and str(previous["person"].get("sha256") or "") == source_hashes["person"]
    ):
        exception = unchanged_source_exception()
        if not exception:
            raise RuntimeError(
                "今日台獎來源尚未更新：store/person SHA-256 與前次正式 run 完全相同；"
                "不得以 staging mtime、檔名或舊 outputs 發布"
            )
    return {
        "run_id": run_id,
        "report_run_date": report_run_date.isoformat(),
