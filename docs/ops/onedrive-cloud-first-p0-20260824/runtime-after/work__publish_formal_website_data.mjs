...
export function canonicalAwardsSourceFiles(value, label = 'awards') {
  ...
  for (const [key, expectedBasename] of Object.entries(REQUIRED_AWARDS_SOURCE_BASENAMES)) {
    ...
    if (provider !== 'onedrive-cloud' || !driveItemId || !eTag
      ...
      fail(`${label} ${key} OneDrive cloud identity is incomplete`);
    }
    if (!runId) {
    ...
    canonical[key] = {
      ...
      lastModifiedDateTime, eTag, size, sha256, source_data_date: sourceDataDate, run_id: runId,
    };
  }
  return canonical;
}
...
function assertAwardsSourceFilesEqual(actual, expected, label) {
  ...
  for (const key of Object.keys(REQUIRED_AWARDS_SOURCE_BASENAMES)) {
    for (const field of [
      ...
      'eTag', 'size', 'sha256', 'source_data_date', 'run_id',
    ]) {
