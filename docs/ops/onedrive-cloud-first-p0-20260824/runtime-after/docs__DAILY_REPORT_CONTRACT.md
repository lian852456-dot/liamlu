...
1. 透過 Microsoft Graph 直接列出 OneDrive `TWM每日戰報` 雲端資料夾，精準選取同日 KPI 與兩份 canonical 台獎來源，下載 bytes 到 run-scoped staging 並驗證 SHA-256。
2. `REPORT_DATE_ISO=YYYY-MM-DD node report-automation/work/run_daily_north12b_report.mjs`；production 預設 `REPORT_SOURCE_MODE=onedrive-cloud`，KPI component 只要求自己的 canonical source/date gate，不被 stale awards 阻擋。
3. 先執行 `REPORT_DATE_ISO=YYYY-MM-DD REPORT_DATA_CUTOFF_DATE=YYYY-MM-DD node report-automation/work/preflight_onedrive_cloud_sources.mjs`。只有 KPI／店點／個人三份 Excel 截止日相同且兩份台獎 cloud version 都 fresh 時，才以同一 run manifest 執行 `python3.12 report-automation/work/update_phone_awards.py`。
