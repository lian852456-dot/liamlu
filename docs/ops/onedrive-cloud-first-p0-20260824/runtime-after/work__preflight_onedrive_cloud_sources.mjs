...
export async function loadOneDriveGraphAccessToken() {
  const direct = String(process.env.ONEDRIVE_GRAPH_ACCESS_TOKEN || '').trim();
  if (direct) return direct;
  const tokenFile = String(process.env.ONEDRIVE_GRAPH_ACCESS_TOKEN_FILE || '').trim();
  if (tokenFile) {
    const value = String(await fs.readFile(path.resolve(tokenFile), 'utf8')).trim();
    if (value) return value;
  }
  const service = String(process.env.ONEDRIVE_GRAPH_KEYCHAIN_SERVICE || 'North12BOneDriveGraphAccessToken').trim();
  const account = String(process.env.ONEDRIVE_GRAPH_KEYCHAIN_ACCOUNT || process.env.USER || '').trim();
  if (!account) blocked('Graph Keychain account is missing');
  try {
  ...
  blocked(`Graph credential is unavailable (Keychain service ${service}); local fallback is OFF`);
}
...
export async function runOneDriveCloudPreflight({
  ...
  const client = graphClient || new OneDriveGraphClient({ accessToken: await loadOneDriveGraphAccessToken() });
