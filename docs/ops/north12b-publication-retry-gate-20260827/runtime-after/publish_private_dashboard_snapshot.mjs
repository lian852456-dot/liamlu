...
const sourceMode = process.env.REPORT_SOURCE_MODE || '';
const sourceDir = process.env.PRIVATE_DASHBOARD_SOURCE_DIR || '';
const reuseVerifiedArtifacts = process.env.PRIVATE_DASHBOARD_REUSE_VERIFIED_ARTIFACTS === '1';
let activePublicationRunId = kpiRunId;
...
async function post(payload) {
  const actionName = String(payload?.action || 'publish');
  const body = await postPublicationJson({
    ...
    component: kpiOnly ? 'KPI' : (awardsOnly ? 'awards' : 'KPI+awards'),
    action: actionName.includes('status') ? 'website-readback' : 'publish',
  });
...
async function main() {
  ...
  try {
    ...
    if (awardsOnly) {
      ...
      activePublicationRunId = String(awardsBattle.source_files?.store?.run_id || '');
      ...
      for (const kind of ['store', 'person']) {
        ...
        if (!source || !['onedrive-cloud', 'google-drive-cloud'].includes(provider)
            || !source.driveItemId || (provider === 'onedrive-cloud' && !source.eTag)
            || !source.lastModifiedDateTime || source.source_data_date !== dataCutoffDate
            || source.run_id !== awardsBattle.source_files.store.run_id) {
          throw new Error(`local awards ${kind} cloud identity is incomplete`);
        }
      }
      const preflightStatus = await post({ action: 'private_admin_snapshot_status', adminSecret });
      assertEqual(preflightStatus.ownerEmail, expectedOwnerEmail, 'private dashboard preflight owner');
    ...
    // A cloud-only report may never silently rebuild from the legacy OneDrive default.
    if (sourceMode === 'google-drive-cloud' && !sourceDir) {
      throw new Error('Google Drive cloud mode requires PRIVATE_DASHBOARD_SOURCE_DIR');
    }
    const websiteBuildArgs = [path.join(reportWorkDir, 'build_github_pages_data.py')];
    if (sourceDir) websiteBuildArgs.push('--source-dir', sourceDir);
    await run(bundledPython, websiteBuildArgs);
    ...
    activePublicationRunId = String(kpiBattle.kpi_run_id || awardsBattle.source_files?.store?.run_id || '');
    const awardConfig = await loadActiveAwardConfig();
