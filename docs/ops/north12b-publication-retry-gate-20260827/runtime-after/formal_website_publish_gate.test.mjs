...
import { PrivateDashboardTransportError } from './private_dashboard_transport.mjs';
...
const AWARDS_RUN_ID = 'fixture-20260808-awards';
const cloudAwardIdentity = (kind, basename, sha256, lastModifiedDateTime) => ({
  provider: 'onedrive-cloud',
  driveItemId: `${kind}-drive-item-id`,
  basename,
  canonical_basename: basename,
  lastModifiedDateTime,
  eTag: `${kind}-etag-v2`,
  size: kind === 'store' ? 123456 : 234567,
  sha256,
  source_data_date: '2026-08-07',
  run_id: AWARDS_RUN_ID,
});
const AWARDS_SOURCES = Object.freeze({
  store: cloudAwardIdentity(
    'store',
    '01-08-03-(密)直營_手機競賽日報_店點達成率、排名及獎金.xlsx',
    'a'.repeat(64),
    '2026-08-07T01:40:28Z',
  ),
  person: cloudAwardIdentity(
    'person',
    '01-08-04-(密)直營_手機競賽日報_個人達成率、排名及獎金.xlsx',
    'b'.repeat(64),
    '2026-08-07T01:40:56Z',
  ),
});
...
function fixture() {
  ...
  const awardsBattle = {
    ...
    processing_run_id: AWARDS_RUN_ID,
  ...
  return {
    kpicalcData: {
      meta: {
        month: '2026-08', snapshotDay: 7, sourceFile: '0808.xlsx',
        reportDate: '2026-08-08', processingRunId: AWARDS_RUN_ID,
      },
      stores,
    ...
    snapshot: {
      kpiBattle: {
        ...
        kpi_run_id: AWARDS_RUN_ID, processing_run_id: AWARDS_RUN_ID,
    ...
    expected: {
      reportRunDate: '2026-08-08', reportDate: '2026-08-07', dataAsOfDate: '2026-08-07', sourceFile: '0808.xlsx',
      processingRunId: AWARDS_RUN_ID,
...
test('KPI 0822.xlsx 與獨立台獎來源不會覆蓋 0821 正式資料截止日', () => {
  ...
  value.kpicalcData.meta.reportDate = '2026-08-22';
  ...
  value.snapshot.awardsBattle.source_files.store.source_data_date = '2026-08-21';
  value.snapshot.awardsBattle.source_files.person.source_data_date = '2026-08-21';
  ...
  value.expected.reportRunDate = '2026-08-22';
...
test('Google Drive cloud source_files 可通過正式 readback，且不要求 OneDrive eTag', () => {
  const value = fixture();
  for (const entry of Object.values(value.snapshot.awardsBattle.source_files)) {
    entry.provider = 'google-drive-cloud';
    entry.googleDriveFileId = entry.driveItemId;
    delete entry.eTag;
  }
  value.expected.awardsBattle = structuredClone(value.snapshot.awardsBattle);
  const result = validateDualFormalReadback(value);
  assert.equal(result.dashboard.awardVerification.sourceFiles.store.provider, 'google-drive-cloud');
  assert.equal(result.dashboard.awardVerification.sourceFiles.person.googleDriveFileId,
    result.dashboard.awardVerification.sourceFiles.person.driveItemId);
});
...
test('KPI、Dashboard 與兩支台獎不同 processing run identity 時必須 blocked', () => {
  for (const mutate of [
    value => { value.kpicalcData.meta.processingRunId = 'stale-kpi-run'; },
    value => { value.snapshot.kpiBattle.kpi_run_id = 'stale-dashboard-run'; },
    value => { value.snapshot.awardsBattle.processing_run_id = 'stale-awards-run'; },
    value => { value.snapshot.awardsBattle.source_files.person.run_id = 'stale-person-run'; },
  ]) {
    const value = fixture();
    mutate(value);
    assert.throws(() => validateDualFormalReadback(value), /(processing run ID|source run_id) mismatch/);
  }
});

test('KPI meta reportDate 缺失時不得以 cutoff 日期或前端補字冒充完成', () => {
  const value = fixture();
  delete value.kpicalcData.meta.reportDate;
  assert.throws(() => validateDualFormalReadback(value), /kpicalc report run date mismatch/);
});
...
test('正式 readback transient HTTP error 有限重試，最終仍以精確驗證結果為準', async () => {
  let calls = 0;
  const retries = [];
  const result = await withBoundedReadbackRetry(async () => {
    calls += 1;
    if (calls < 3) throw new PrivateDashboardTransportError('temporary HTTP error', { status: 503 });
...
test('正式 readback business validation failure 不重試且 fail-closed', async () => {
  let calls = 0;
