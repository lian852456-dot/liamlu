...
import { isTransientPrivateDashboardError } from './private_dashboard_transport.mjs';
...
let publicationContext = { cutoff: '', runId: '' };
...
export async function withBoundedReadbackRetry(readback, {
  ...
  for (let attempt = 1; attempt <= attempts; attempt += 1) {
    try {
      ...
      lastError = error;
      if (attempt === attempts || !isTransientPrivateDashboardError(error)) throw error;
...
export function canonicalAwardsSourceFiles(value, label = 'awards') {
  ...
  for (const [key, expectedBasename] of Object.entries(REQUIRED_AWARDS_SOURCE_BASENAMES)) {
    ...
    const driveItemId = String(entry.driveItemId || '');
    const lastModifiedDateTime = String(entry.lastModifiedDateTime || '');
    const eTag = String(entry.eTag || '');
    const size = Number(entry.size);
    const sourceDataDate = String(entry.source_data_date || '');
    ...
    if (!['onedrive-cloud', 'google-drive-cloud'].includes(provider) || !driveItemId
        || (provider === 'onedrive-cloud' && !eTag)
        || !lastModifiedDateTime || Number.isNaN(Date.parse(lastModifiedDateTime))
        || !Number.isSafeInteger(size) || size <= 0 || !/^\d{4}-\d{2}-\d{2}$/.test(sourceDataDate)) {
      fail(`${label} ${key} cloud identity is incomplete`);
    }
    if (!runId) {
      fail(`${label} ${key} source run_id is missing`);
    }
    canonical[key] = {
      provider, driveItemId, basename: source, canonical_basename: source,
      lastModifiedDateTime, ...(eTag ? { eTag } : {}), size, sha256,
      source_data_date: sourceDataDate, run_id: runId,
...
function assertAwardsSourceFilesEqual(actual, expected, label) {
  ...
  for (const key of Object.keys(REQUIRED_AWARDS_SOURCE_BASENAMES)) {
    const fields = [
      'provider', 'driveItemId', 'basename', 'canonical_basename', 'lastModifiedDateTime',
      'size', 'sha256', 'source_data_date', 'run_id',
    ];
    if (actualCanonical[key].provider === 'onedrive-cloud') fields.push('eTag');
    if (actualCanonical[key].provider === 'google-drive-cloud') fields.push('googleDriveFileId');
...
async function validateManifestSourceBatch({ manifest, today, reportRunDate }) {
  const recorded = manifest && manifest.sourceBatch;
  if (!recorded || typeof recorded !== 'object') fail('same source batch record is missing');
  const validated = validateSameSourceBatch({
    reportRunDate,
    kpi: recorded.kpi,
    awards: recorded.awards,
  });
  assertEqual(String(recorded.batch_id || ''), validated.batch_id, 'same source batch id');
  const sourcePath = String(today && today.source_path || '');
  if (!sourcePath) fail('same source batch KPI source path is missing');
  const currentHash = createHash('sha256').update(await fs.readFile(sourcePath)).digest('hex');
  assertEqual(validated.kpi.sha256, currentHash, 'same source batch KPI SHA-256');
  return validated;
}
...
export function assessCurrentKpicalc({ kpicalcData, kpiSnapshot, expected }) {
  try {
    ...
    assertEqual(String(kpicalcData.meta.reportDate || ''), expected.reportRunDate, 'kpicalc report run date');
    assertEqual(String(kpicalcData.meta.processingRunId || ''), expected.processingRunId, 'kpicalc processing run ID');
...
export function validateDualFormalReadback({ kpicalcData, snapshot, snapshotStatus, expected }) {
  ...
  assertEqual(String(kpicalcData.meta.reportDate || ''), expected.reportRunDate, 'kpicalc report run date');
  assertEqual(String(kpicalcData.meta.processingRunId || ''), expected.processingRunId, 'kpicalc processing run ID');
  assertEqual(String(snapshot.kpiBattle.report_run_date || ''), expected.reportRunDate, 'dashboard KPI report run date');
  assertEqual(String(snapshot.awardsBattle.report_run_date || ''), expected.reportRunDate, 'dashboard awards report run date');
  assertEqual(String(snapshot.kpiBattle.processing_run_id || snapshot.kpiBattle.kpi_run_id || ''), expected.processingRunId, 'dashboard KPI processing run ID');
  if (snapshot.kpiBattle.processing_run_id && snapshot.kpiBattle.kpi_run_id) {
    assertEqual(String(snapshot.kpiBattle.kpi_run_id), expected.processingRunId, 'dashboard KPI legacy processing run ID');
  }
  assertEqual(String(snapshot.awardsBattle.processing_run_id || ''), expected.processingRunId, 'dashboard awards processing run ID');
  ...
  assertEqual(String(snapshot.awardsBattle.source_files?.store?.run_id || ''), expected.processingRunId, 'dashboard store awards processing run ID');
  assertEqual(String(snapshot.awardsBattle.source_files?.person?.run_id || ''), expected.processingRunId, 'dashboard personal awards processing run ID');
  ...
  return {
    ...
    reportRunDate: expected.reportRunDate,
...
async function post(endpoint, payload) {
  const body = await postPublicationJson({
    ...
    cutoff: publicationContext.cutoff,
    runId: publicationContext.runId,
    component: String(payload?.action || '').includes('kpicalc') ? 'KPI' : 'KPI+awards',
...
async function writeManifest(publication, details = {}) {
  ...
  if (publication.result === 'published-verified') {
    ...
    manifest.snapshotPublishedAt = details.snapshotPublishedAt || manifest.snapshotPublishedAt || null;
...
async function main() {
  ...
  const sourceBatch = await validateManifestSourceBatch({ manifest, today, reportRunDate });
  assertEqual(sourceBatch.data_cutoff_date, dataCutoffDate, 'same source batch data cutoff date');
  const expected = {
    reportRunDate,
    reportDate: dataCutoffDate,
    dataAsOfDate: dataCutoffDate,
    sourceFile: basename(today.source_file || today.source_path),
    processingRunId: String(sourceBatch.kpi.run_id || ''),
  ...
  publicationContext = { cutoff: dataCutoffDate, runId: expected.processingRunId };
  ...
  assertAwardsSourceFilesEqual(sourceBatch.awards, expected.awardsSourceFiles, 'same source batch awards');
  ...
  try {
    if (!verifyOnly) {
      ...
      snapshotResult = await runJson(path.join(workDir, 'publish_private_dashboard_snapshot.mjs'), [], {
        ...
        REPORT_SOURCE_MODE: process.env.REPORT_SOURCE_MODE || '',
        PRIVATE_DASHBOARD_SOURCE_DIR: process.env.PRIVATE_DASHBOARD_SOURCE_DIR || '',
      });
    }
