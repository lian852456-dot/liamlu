...
if ! load_report_upload_identity; then
  print -u2 "formal website publish blocked: upload identity configuration is missing or invalid."
  exit 78
fi
...
elif (( ${publisher_args[(I)--awards-component-only]} )); then
  publisher="$PROJECT_ROOT/report-automation/work/publish_awards_component_data.mjs"
  publisher_args=("${publisher_args[@]:#--awards-component-only}")
fi
