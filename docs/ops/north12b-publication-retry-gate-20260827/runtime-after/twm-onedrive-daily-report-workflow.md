...
- 好速上線明細：只取寄送日／report run date 前一日。例如 `0825.xlsx` 只顯示 `8/24`，不可顯示 8/1～8/23 月累計明細；若 8/24 為 0 筆，頁籤註記 `2026-08-24 當日無上線` 並略過明細列，不得 fallback 到較舊日期。
- 好速加碼：自 `2026-08-27` 戰報（資料日 `2026-08-26`）起，認列的 `500M`、`1G`、`1.2G/1G+` 逐筆增加 `0.5` 戰報點數；剔除列不加。來源 `業績認列` 已含 36M `+0.5`，36M 欄只供拆分稽核，不得重複加總。Excel 必須同時列出來源認列、36M 已含加碼、1G/500M 戰報加碼與戰報總點數。
- M+ 後送清單：同一個寄送前檢查會另外產出 `/Users/liamlu/Downloads/liam-agent/report-automation/outputs/mplus_delivery_YYYY-MM-DD.json`。manifest 固定只列出 M+ 對話 `滷蛋` 所需的 4 個附件：當日 KPI 戰報 Excel、主力 KPI 圖、加掛項加減分圖、台獎圖片；初始狀態為 `waiting-mail-verification`，不得在 Outlook 兩封郵件完成 `寄件備份` 驗證前上傳。
- M+ best-effort 規則：Outlook 兩封信都完成 `寄件備份` 驗證後、網站更新前，執行 `node report-automation/work/mplus_best_effort_delivery.mjs start --report-date YYYY-MM-DD`。只有輸出 `should_attempt=true` 才可使用既有 Chrome / M+ Web 登入狀態開啟 `https://web.mplusapp.com/chat.do`、選取 `滷蛋` 並上傳 manifest 的 KPI Excel 與三張圖片。對話尾端 4 個附件都顯示 `已傳送` 時，以 `finish --status sent-verified --reason-code thread-tail-verified` 記錄；登入、重複登入、file chooser、安全政策或上傳驗證受阻時，以 `manual-confirmation-required`、`partial` 或 `failed` 加上不含敏感資訊的 reason code 記錄。
- M+ 隔離規則：M+ 是完全獨立、best-effort 的附加步驟。任何 M+ 問題都不得重寄、回滾或降低 Outlook `寄件備份` 的成功狀態，也不得阻擋、略過或改變後續網站更新與正式 readback。M+ helper 的錯誤會輸出 `non_blocking=true` 並以 exit code 0 結束；網站 closeout 必須照常繼續。M+ 完成仍須以對話尾端 `已傳送` 為獨立證據，manifest 或 `attempting` 不等於已送達。
...
- 09:45 P0 credential preflight：在任何 cloud source、產報與寄信前，必須先以乾淨環境執行 `automation_environment_preflight_with_keychain.sh`。`REPORT_UPLOAD_EMPLOYEE_ID` 只可由 macOS Login Keychain service `North12BReportUploadEmployeeId` 載入，格式固定為 5–12 碼英數字；實際值不得輸出或落盤。缺少／格式錯誤、管理者 Keychain、Node、curl、endpoint 或 connector auth 不可用均為 non-retryable configuration failure，必須在 source/build/mail 前 BLOCKED，所有後段 invocation count 為 0。
- 私有網站雙發布：僅在每日戰報與台獎兩封 Outlook 郵件皆完成 `寄件備份` 驗證後，才可執行。先以 `build_github_pages_data.py --report-run-date <執行／寄信日> --data-cutoff-date <source_date_range 最後一日>` 建立本機快照，確認 KPI／台獎 `report_run_date` 等於執行／寄信日，`report_date` 與 KPI `data_as_of_date` 等於資料截止日；不得從 `MMDD.xlsx`、郵件或附件檔名推導正式資料日。再執行 `REPORT_RUN_DATE_ISO=<執行／寄信日> REPORT_DATA_CUTOFF_DATE=<資料截止日> zsh /Users/liamlu/Downloads/liam-agent/report-automation/work/publish_formal_website_with_keychain.sh`；wrapper 會自行從同一 Keychain service 載入員編，不得要求互動式 shell 預先 export。流程固定先更新既有 `north12b-kpicalc-private-latest.json`，再更新既有 `north12b-dashboard-private-latest.json`；不得修改網站、重新部署 GAS 或變更 JSON 結構。快速上傳解析後必須把 `meta.sourceFile` 校正為原始 `MMDD.xlsx`，不可保留 `report-upload-temp-*` 暫存檔名。manifest 檔名仍以執行日定位，但正式 publication/readback 的 `reportDate` 必須等於資料截止日；任一日期或來源不一致都維持 blocked。
