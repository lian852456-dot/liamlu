...
| 5 | KPI 端到端真實驗證 | 解析層已用真實 0730.xlsx 驗證（§6）；**寫入／發佈端仍只有 Liam 能驗** |
...
## 6. KPI 樣本：0730.xlsx 已驗證（2026-07-31）

Liam 提供 Drive 的 `0730.xlsx`（來源資料夾即 `KPICALC_SOURCE_FOLDER_ID_DEFAULT`）。
...
1. 一份**含** `上線數KPI_個人達成率_明細` 的日報 —— `detail` 主路徑至今仍未用真實檔驗證過。
2. 確認 `.xls` 是否真的還會出現；若否，直接不做。

### 注意：檔名日期 ≠ 資料日期

`0730.xlsx` 的資料期間是 `~ 07/29`，即 `dataDate = 2026-07-29`。
版本比較一律以 **meta 解析出的資料日期**為準，不看檔名。
