...
- 修復：隔離 Patrol GAS 的 `ptdashboard` 現與前端 25 題模型一致，將第 1 項「督導打卡」保留為顯示用的非必要項目。缺它時不再出現在缺項、8/9 或阻擋巡店完成；已符合其餘題目及兩次相隔七日條件的六張犁，會顯示完成。
- 發布與讀回：功能提交 `b345901` 已進 `main`，正式 Pages `patrol.html` 回讀為題目版本 `v=5`、面談模型 `v=2`、看板 cache contract `v2`。既有 Patrol Web App 原部署已更新為 GAS 第 11 版，Deployment ID 與存取權限不變。
- 驗證：Patrol／面談完整 Node 套件 142/142 PASS、`npm run build:patrol-gas` 與 `git diff --check` PASS。未用真人面談檔寫入正式資料；實際檔案匯入讀回與使用者裝置驗收仍待 Liam 重試確認。
