...
- 結果（本機候選／未部署）：Node 全站 `358/358`、Patrol Chromium `80/80`、`git diff --check` 通過。全站 Chromium 為 `195/203`；失敗 8 案已在乾淨 `origin/main` 以相同檔案重跑並逐案完全重現（基準 `12/20`、同 8 案），故不列為本次回歸。未 commit、未 push、未部署 Pages／GAS、未讀寫正式資料；本機 `file://` 預覽重載受瀏覽器安全規則阻擋，Liam 眼前分頁的視覺驗收與正式 7／8 月 readback 尚待完成。
- 經驗 / 給下一位的提醒：巡店次數以 `arriveTime` 日期為準，缺值才用 `fillTime`；同日多題／重複列只算一次。`ptsummary` 仍為既有 33 題摘要，9 月新版 25 題與雙次間隔由 `ptdetail` 前端模型計算。GAS source／bundle 雖已修正萬大代碼，正式 GAS 未部署，不能宣稱線上已同步。
