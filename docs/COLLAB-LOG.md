...
## 2026-07-15 ｜ Codex
- 做了什麼：在 `patrol.html` 新增通行碼保護的 `每月班表` 與 `半月督導檢查` 頁籤；從 OneDrive `TWM 班表` 的 9 份 `.xls` 產生 `data/schedule.js` / `data/schedule.json`，並新增每日、每週、每月檢視與 Excel 匯出。半月檢查支援 33 題、異常改善說明、照片／影片附件名稱、本機暫存與 GAS `hread` / `hwrite` 雲端同步。
- 結果：成功。原有巡店、新增頁籤與主站測試共 31 項全部通過；GAS 變更尚待 Apps Script 新版本部署後由瀏覽器驗證。
- 經驗 / 給下一位的提醒：照片／影片原檔不直接寫入 Google Sheets，避免儲存格與 URL 長度限制；試算表保存附件名稱，原始媒體留在填寫裝置。部署 `gas/Code.gs` 後要確認 `hread` / `hwrite` 已在新版本生效。
- 做了什麼：先試作 `📈 區間彙整` 與 `🏅 台獎提醒`；從 OneDrive 日期報表的 `上線數KPI_每日上線` 解析北一二B整體＋9店的日動能，並把台獎頁與填報／回放頁的機款欄位統一為目前協作的 10 台機款。
- 結果：成功。新增網站資料檔 `data/daily-momentum.json`、`data/phone-awards-latest.json`，加入 SVG 趨勢圖、近3日下滑、掛蛋與台獎缺口提醒；Playwright 29 項測試全數通過。
- 經驗 / 給下一位的提醒：日動能不能用門市手動回填判斷，應以 OneDrive `上線數KPI_每日上線` 為準；空白不算掛蛋，只有明確 0 才算。舊台獎欄位保留在 GAS 歷史資料，不從前端新回報欄位顯示。
