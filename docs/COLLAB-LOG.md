...
## 🔴 進行中／待辦（2026-07-29 更新第二次，接手者先看這段）

**狀態：Liam 已完成貼碼＋部署，自動化真的在跑了——但漏了一個服務開關，今天(0729)那次自動更新失敗。**

| 項目 | 狀態 |
|---|---|
| repo 程式碼 | ✅ 已貼進 GAS 並部署（`gas/Code.gs` 1968 行版本；後續 merge 到 1975 行，含 Codex 的 savedAt/半月附件修正） |
| 通行碼 | ✅ Liam 已在 GAS 填入實際密碼（不是 `CHANGE_ME` 了） |
| 11:00 自動更新 | ⚠️ **0729 這次失敗**：`Drive is not defined`——**不是日報格式問題**，是 GAS 專案沒加「Drive API」進階服務，`Drive.Files.create()`（xlsx→Sheets 轉檔用）找不到全域物件 |
| 12:30 巡檢 | 應該已啟用（Liam 有跑 `setupKpiCalcWatchdog`），今天應該會收到「可能未更新」提醒信，屬正常 |
| 同仁看到的資料 | ✅ 已產到累計 **07/28**（0729 日報 AI 解析完成，9/9 店與報表逐項吻合，已交 Liam 走督導發佈區手動補上） |
| patrol.html／home.html／督導試算區 | ✅ 全部上線（`main` 已推送，網址見下方） |

**待 Liam 執行（修 Drive API，約 2 分鐘，一次性設定）**：
1. 打開 GAS 編輯器，左側「服務」（Services）旁的 **＋**
2. 找「Drive API」，選「Drive API」加入（識別碼會顯示 `Drive`）
3. 存檔（**不用重新部署**——這是時間觸發器要用的服務，不是 `doGet`/`doPost` 改動）
4. 函式選 `testKpiCalcAutoUpdate` 手動執行一次確認會成功（收到 ✅ 信）

**給下一位的提醒**：這個服務理論上 07-19/07-20 就啟用過（那時自動更新正常跑過），
這次會消失代表**要嘛是新建了 GAS 專案沒帶過去、要嘛是設定被動過**——貼 `Code.gs` 進 GAS
編輯器**不會**動到這個服務清單（它是專案層級設定，不在 `Code.gs` 檔案內），所以每次交接
新 GAS 專案，除了貼碼跟六步驟，**記得額外提醒加 Drive API 進階服務**，我這次就漏講了。
