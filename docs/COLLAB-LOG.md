...
## 2026-07-20 ｜ Claude（訂立三層驗證規範：Drive 直讀試算表）
- 做了什麼：確認 Claude 遠端環境可經 Google Drive 連接器直接讀「北一二B每日回報」試算表
  （proxy 仍封鎖 script.google.com，但 Drive API 不受影響；實測讀回「巡店明細」全表約 16 萬字元）。
  依 Liam 指示把驗證方式寫成三層規範進 CLAUDE.md／AGENTS.md：(1) 試算表資料驗證→AI 直讀，
  不再請 Liam 人工複製 JSON；(2) GAS 部署版本驗證→仍需 Liam 開 `?action=debug`；
  (3) 正式網站端到端操作→需 Liam 實機測試。第 2、3 層人工項目須集中成一次性驗收清單，
  不逐項來回。「常用檢查清單」步驟 4、5 同步改寫。
- 結果：成功（純文件改動，無程式碼變更）。
- 經驗 / 給下一位的提醒：**試算表有資料 ≠ 最新 GAS 已部署 ≠ 正式站正常**，三層不可互相替代。
  Codex 等無 Drive 連接器的環境不適用第 1 層直讀，照舊流程。
