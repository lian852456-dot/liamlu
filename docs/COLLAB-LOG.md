...
## 2026-07-22｜Claude｜【交接狀態】GAS savedAt 日期顯示修正

> **狀態：程式修正與部署文件已完成，等待正式 Apps Script 部署與驗收。**
> ⚠️ **尚未部署；正式站尚未宣告修復；PR 尚不可合併。**

**基本資訊**
- 任務名稱：GAS savedAt 日期顯示修正
- 分支：`claude/gas-savedat-date-fix-qakgt8`
- PR：**#23，Open，未 merge**
- 主要修正 commit：`30052b0`
- 人工部署文件 commit：`3774e5d`
- 部署檢查清單 commit：`a569127`

**根因**
- savedAt 在試算表是「純時間」值，`getValues()` 回傳 1899-12-30 基準的 Date；
  `readData()` 原本一律走 `toDateStr()`（yyyy-MM-dd）→ 被格式化成「1899-12-30」。
- 修法：`readData()` 對 savedAt 欄改用 `getDataRange().getDisplayValues()` 原始顯示字串
  （如 `16:00:00`）。只動 `readData()` 一處，寫入邏輯與其他欄位不變。

**已完成項目**
- 根因確認
- 最小程式修正（`gas/Code.gs` `readData()`）
- GAS 日期回歸測試 3/3 passed（`tests/gas-date-regression.js`）
- 完整 Playwright 40 passed／2 個既有失敗（patrol，與本次無關，零回歸）
- `VALIDATION_REPORT.md`（驗證與交接報告）
- `MANUAL_GAS_DEPLOY.md`（無 clasp 人工部署指南）
- `GAS_DEPLOY_CHECKLIST.md`（部署前後把關清單）
- PR #23（Open）

**未完成項目**
- 正式 Apps Script 部署
- 2026-07-20 16:00 的 9 筆正式資料驗收
- 第二裝置跨裝置寫入、彙整、回放與個人追蹤驗收
- 驗收成功後才可決定是否 merge PR #23

**部署缺少條件**
- Apps Script 編輯權限
- 正式 Apps Script 專案識別資訊（Script ID／部署 ID，repo 內無）
- 可建立新版本並部署的人員或環境（本容器 proxy 擋 `script.google.com`）

**正式驗收條件（全部成立才算修好）**
- savedAt 不得出現 1899-12-30
- 9 筆資料皆可讀取（`?action=read&date=2026-07-20&seg=16`）
- 日期、彙整、回放功能正常

**Rollback**
- Apps Script「管理部署作業」切回前一版本（最快，不需改碼）
- Git：`git revert 30052b0`

**給下一位的提醒**
1. ⚠️ 接手時發現前一 session「本機已改好」的變更並不存在——遠端環境每次重新 clone，
   未 commit/push 的工作會遺失。**改完務必 commit＋push。**
2. **部署只換 `readData()`，勿整檔覆蓋線上 Code.gs**（可能洗掉別人未進 repo 的更新）；
   存檔≠部署，須建立「新版本」（CLAUDE.md 坑 #3）。詳見 `GAS_DEPLOY_CHECKLIST.md`。
3. **未部署＋未驗收前，不得宣稱正式站已修好，PR #23 不可合併。**
