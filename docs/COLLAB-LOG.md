...
- 結果：把正式報銷表 11 條路線餵進計算引擎，**10/11 逐日完全相符**；
  6/15 三店逐段 4.4＋10.0＝14.4（非平均）。Playwright 27 passed / 2 failed（既有）。
- 經驗 / 給下一位的提醒：
  * **⚠️ 6/22 待裁示**：正式報銷表寫「台北永吉/台北復興南 6.5」，但巡店明細
    (`20260623.csv`) 與 `Y2606月車資.xls` 均為「台北大稻埕/台北復興南 6.5」，
    且 **永吉↔復興南 在十個月車資資料中完全沒有觀測**。依「不可猜測」原則未建立該路段，
    引擎對這天回傳「待查」。若確認為大稻埕，合計即為 68.0＋6.5＝74.5 KM 完全吻合。
  * **成本歸屬部門／成本歸屬者尚無實際值**（照片未傳達），已做成可填欄位並留白，
    匯出前會提示。`MI.setCost()` 可設定，存 localStorage。
