...
## 🔴 進行中／待辦（2026-07-29 更新，接手者先看這段）

**狀態：repo 這邊全部就緒，卡在 Liam 還沒把 `gas/Code.gs` 貼進 GAS 編輯器。**
自動更新目前仍是「壞的」，靠人工每天補。

| 項目 | 狀態 |
|---|---|
| repo 程式碼 | ✅ 全部就緒（`gas/Code.gs` **1968 行**，含自動化＋通知信業績重點＋日報格式回退＋通行碼） |
| 同仁看到的資料 | ✅ 已產到累計 **07/27**（0728 日報解析完成，待 Liam 上傳發佈） |
| GAS 每日 11:00 自動更新 | ❌ **失效**（編輯器是舊版碼，`kpiCalc*` 函式不存在 → 空轉且不寄失敗信） |
| 12:30 巡檢守門員 | ❌ **從未啟用**（這就是先前 4 天停更沒人發現的原因） |
| 日報格式變動 | ✅ 已處理（0728 起可能缺 `_個人達成率_明細`，解析器已加回退並交叉驗證） |
| patrol.html 改名 | ✅ 已改「**Liam情報站**」（純前端） |
| 巡店通行碼 | ⚠️ 程式碼已改回真的檢查，但 **repo 是 `CHANGE_ME` 佔位字**，等 Liam 在 GAS 填實際密碼 |
| 導覽首頁 home.html | ✅ 已上線（純靜態，不影響 GAS） |

**待 Liam 執行（只有他能做，約 5 分鐘，一次做完）**：
1. 貼 repo main 最新 `gas/Code.gs`（**貼前照 CLAUDE.md 的 grep 清單自我驗證版本**）→ 存檔
2. **把 `PT_KEY` 從 `CHANGE_ME` 改成他自己的密碼**（不改的話巡店會擋掉所有人，包含他自己）
3. 函式選 `testKpiCalcAutoUpdate` → 執行（會寄 ✅ 信；**選單看不到此函式＝貼到舊版**）
4. 執行 `setupKpiCalcAutoUpdate`（重建 11:00 排程）
5. 執行 `setupKpiCalcWatchdog`（**啟用巡檢，別再漏**）
6. 部署 → 管理部署作業 → ✏️ → **新版本** → 部署（讀檔新邏輯＋督導試算區 isTrusted＋通行碼靠它）

**在上述完成前的每日暫行流程**：Liam 把日報丟進 Drive → 請 AI 解析產生 JSON →
Liam 從 kpi.html 進階「督導發佈區」上傳（約 2 分鐘）。
※ 或由 AI 直接寫私有 Drive 補 `north12b-kpicalc-<日期>.json`——**但此路徑要等步驟 6 部署後才生效**。
