...
## 2026-07-22｜Claude｜修正 savedAt 顯示 1899-12-30（分支，未部署）

- 做了什麼：`gas/Code.gs` 的 `readData()` 對 `savedAt` 欄改用 `getDataRange().getDisplayValues()`
  的原始顯示字串（如 `16:00:00`）。原因：savedAt 在試算表是「純時間」值，`getValues()`
  會回傳 1899-12-30 基準的 Date，先前一律走 `toDateStr()` 被格式化成「1899-12-30」。
  只動 `readData()` 一處，其餘欄位與寫入邏輯不變。新增 `tests/gas-date-regression.js`
  （node vm 沙箱載入真實 Code.gs 直呼 readData 驗證）。
- 結果：**進行中——已在分支 `claude/gas-savedat-date-fix-qakgt8` 完成並測試，尚未部署正式 GAS。**
  GAS 日期回歸 3/3、完整 Playwright 40/2（2 個 patrol 失敗為既有、與本次無關，零回歸）。
  全報告見根目錄 `VALIDATION_REPORT.md`。
- 經驗 / 給下一位的提醒：
  1. ⚠️ 這次接手時**發現前一 session「本機已改好」的變更並不存在**——遠端環境每次重新 clone，
     未 commit/push 的工作會遺失。**改完務必 commit＋push，別只存在本機。**
  2. repo 內**沒有** `.clasp.json`／`appsscript.json`／Script ID，無法自動部署 GAS；
     本容器 proxy 又擋 `script.google.com`。部署仍須有編輯權者到 Apps Script 編輯器
     **建立「新版本」部署**（存檔≠部署，見 CLAUDE.md 坑 #3），再開瀏覽器 `?action=read` 驗收。
  3. 正式驗收要用 2026-07-20 16:00 的 9 筆歷史資料，確認 savedAt 不再是 1899-12-30、
     且日期/彙整/回放不受影響。**未部署＋未驗收前，不得宣稱正式站已修好。**
