...
| 項目 | 狀態 |
|---|---|
| repo 程式碼 | ✅ 已貼進 GAS 並部署（`gas/Code.gs` 1975 行，含自動化＋通知信＋日報格式回退＋通行碼＋Codex 的 savedAt/半月附件修正） |
| 通行碼 | ✅ Liam 已在 GAS 填入實際密碼（不是 `CHANGE_ME` 了） |
| Drive API 進階服務 | ✅ **Codex 已修好**（見下方驗證方法） |
...
2. **看 Gmail 成功信主旨**：`kpiCalcPublish`（人工上傳）寄的信主旨帶 **「手動發佈｜」**，
   `kpiCalcAutoUpdate`（自動流程）寄的**沒有**這個字樣——這個差異可以精準分辨「這次更新
   是自動流程本身跑通、還是有人手動貼檔案補救」，不用問 Liam 或猜
3. 信件內容跟獨立解析結果（AI 另外重跑一次 make_kpicalc.py）逐店比對數字是否一致
