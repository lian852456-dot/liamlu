...
## 2026-07-17 ｜ Codex（KPI 戰情加入北一二B整體）
- 做了什麼：在受保護的 `🏆 KPI戰情` 店點總覽表首列加入醒目的 `北一二B整體`，並將它加入「各項 KPI 達成」選單；主管可直接查看全區的公司排名、KPI、加掛、A999／A1399／好速／R1399，以及全區各項 KPI 明細。
- 結果：成功。本機資料原本已包含 `aggregate` 的完整指標，故僅補前端呈現與測試，不需另建資料來源。
- 經驗 / 給下一位的提醒：公開填報／彙整頁仍只顯示允許公開的店點回填資料；北一二B完整戰報維持 Apps Script 驗證後的私有 KPI 戰情頁，避免私有日報 JSON 進入公開 GitHub Pages。
