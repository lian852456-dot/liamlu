...
## 2026-07-17 ｜ Codex（台獎 10 款改保留 moto）
- 做了什麼：依 7/16 公告版目標檔與使用者決策，將原本合併的 SONY／moto 項目改為只追 `moto razr fold`；台獎產生器直接讀取每日 01-08-03 的獨立 `razrfold` 實績／目標欄位，維持 `phone_items=10`。同步更新台獎戰報、手機試算工具、網站填報與日期回放的 10 款顯示，並重建私有台獎戰情快照。
- 結果：成功。7/17 北一二B moto 為 3／13、達成 23%，輸出報表、試算工具與本機私有戰情資料均顯示 moto，不再顯示合併品項。
- 經驗 / 給下一位的提醒：moto 的目標與實績已可從每日來源分開取得；獎金級距尚無獨立新版，因此暫沿用原合併 SONY／moto 項目的既有級距。私有快照必須在 Outlook 寄件備份驗證後，且本機具備管理者安全憑證時才可發佈到私有 Drive。

## 2026-07-17 ｜ Codex／Liam（巡店受保護頁籤讀取修復）
- 做了什麼：補齊既有 Apps Script 網頁應用程式的每月班表與半月督導檢查讀取路由；Liam 已將同一份既有部署更新為第 11 版，未變更正式網址、督導碼或既有巡店資料。
- 結果：成功。正式端點的健康檢查回傳 `status: ok`；以無效測試碼呼叫 `sread`、`hread` 均回傳 `unauthorized`，證明新版讀取路由已生效，且權限檢查仍正常運作。
- 經驗 / 給下一位的提醒：這類 `doGet` 路由異動必須用「管理部署作業 → 編輯既有部署 → 建立新版本」發布；不可另建網址。使用者若希望免督導碼，仍須再調整前後端權限邏輯並重新部署。
