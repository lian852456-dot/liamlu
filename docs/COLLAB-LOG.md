...
- 做了什麼：依 Liam 指示盤點 Drive 內既有的 `01-08-03`（店點）與 `01-08-04`（個人）
  手機競賽日報，對照現有台獎資料流與 `awardsBattle` 消費端契約，產出解析器規格草案與
  預覽畫面最小資訊。**本輪只做盤點與規格，沒有寫任何台獎解析程式，沒有動正式資料。**
  詳見 HANDOVER §7。
- 結果：兩份都成功讀取。9 家門市、40 位人員齊全，店代碼與店名與 KPI 日報完全一致，
  資料日期格式也與 KPI 相同（`2026/07/01 ~ 07/29`）。既有測試維持 66/66 與 31/31。
- 經驗 / 給下一位的提醒：
  1. 🚨 **不要用「人工轉錄 base64」的方式把 Drive 檔案落地**。這次試過一次：
     28KB 的 xlsx 轉錄後只剩 30,452／37,796 字元，zip 中央目錄壞掉，`testzip()` 直接丟
     `OSError`。已刪除。小檔案請改用 `read_file_content`（回傳文字），
     大檔案則會被工具自動存到 `tool-results/*.txt`，那條路才可靠。
  2. **因此本輪拿不到合併儲存格數／隱藏工作表／公式數量**——這三項需要二進位檔＋openpyxl。
     文件已標明「未確認」，請在能直接下載的環境補。
  3. **台獎 Excel 有 45 個機款，但網站只顯示 10 款**，而且 `next_label`（下一獎階）與
     `incremental_award`（會增加多少獎金）**兩份 Excel 裡都沒有**。
     這代表獎階／獎金級距表是獨立來源——同資料夾的 `Y26重點台獎手機.xlsx`（412KB）
     看起來就是它。**拿到之前不能寫解析器，否則就是在猜台獎計算規則。**
  4. **個人表的「目標數／達成率」其實是店層級的**（欄名是 `店目標數`、`店推估 達成率`），
     每個人列上重複同一組店數字。做個人達成率時不要誤用成個人目標。
  5. **姓名在來源就已經遮罩**（`X*X` 形式），不需要另外處理；員編有 `558I082` 這種含字母的，
     不要假設純數字。
  6. **區域辨識比 KPI 更可靠**：每列都有固定的 `北一二B` 欄位，可直接過濾，
     不必只靠店名比對。但本樣本沒出現其他區資料，「是否可能混入其他區」仍標記未確認。
  7. 再次確認 **`update_phone_awards.py` / `build_github_pages_data.py` 不在本 repo**
     （`find . -name "*.py"` 只有 `scripts/build_schedule_data.py`，git 歷史也沒有）。
     現有台獎正式資料是私有 Drive 的 `north12b-dashboard-private-latest.json`，
     只有 `index.html` 讀它，沒有 Google Sheet。

## 2026-07-31 ｜ Claude（第四輪：byStore 定為正式格式、暫存檔加固、店名歧義防呆）

- 做了什麼：依 Liam 指示不再把「缺 `_個人達成率_明細`」當待補問題。取 `0728.xlsx` 與
  `0730.xlsx` 兩份真實日報做九點結構比對，把 **byStore 定為第一版正式支援格式**。
  另完成：預覽日期四項顯示、Drive 暫存檔七項加固、店名歧義 `ambiguous-store-match`。
  **樣本全程在倉庫外暫存目錄，未進 git、未推 GitHub、未寫正式資料、未部署。**
- 結果：九點比對 8 項完全一致，1 項發現重要差異（見下）。契約測試 66/66、
  Playwright 31/31、既有 Node 契約 20/20 全綠。
- 經驗 / 給下一位的提醒：
  1. 🚨 **欄位順序在兩份報表之間會變**：`RT降轉率_降轉數(前約 V+D 1399(含)以上)` 等
     **5 個 RT 升降轉率加分項 band 的欄位位置 0728 與 0730 不同**
     （160→148、148→164、164→160、152→168、168→152），名稱集合則完全相同。
     25 項核心 KPI 欄位這兩份剛好相同，**但既然加分項會漂移，就不能假設任何欄位固定**。
     `kpiCalcParseReport` 全程以 band 名稱查表所以沒事——
     **未來任何 normalizer 都不得改用固定欄號取值。這條比「段寬不是 4」更嚴格。**
  2. **檔名日期 ≠ 資料日期，已用兩份確認**：0728→dataDate 2026-07-27、
     0730→dataDate 2026-07-29，都差一天。檔名是產出日、內容是統計截止日。
     預覽畫面現在同時顯示「原始檔名／報表資料日期／上傳時間／是否晚於正式版本」四項＋說明，
     避免使用者以為日期讀錯。
  3. **暫存檔改用固定前綴 `report-upload-temp-` / `report-upload-staging-`**，
     一律建在私有戰情資料夾（**不是** KPI 來源資料夾），並改成 `keepRaw` 旗標＋`finally` 清理，
     所以驗證失敗、解析丟例外、early return 三條路都會清乾淨。
     新增 `reportUploadCleanupTemp(hours)` 清異常殘留（preview 開頭也會順手跑）。
     錯誤紀錄只寫 `fileId=`，不寫檔名或業績內容。
  4. **店名比對加了歧義防呆**：`reportUploadStoreMatch_` 改回傳
     `{status, store, candidates}`，同時命中兩家以上回 `ambiguous-store-match` 且
     **不自行選擇**，由驗證層 block。完全相等可消解歧義（「通化」仍精準命中「通化」）。
     9 家真實店名逐一驗證命中正確項目，外區店名（板橋／中壢／光復）仍為 none。
  5. **測試小坑**：vm sandbox 建立的陣列跨 realm，`assert.deepEqual`（strict 模式）會因
     原型不同而失敗，訊息是「same structure but not reference-equal」。
     比較 sandbox 回傳的陣列請用 `Array.from(x).join()` 或逐項比較。
  6. 附帶發現：KPI 來源資料夾裡有 `01-08-03/04-(密)直營_手機競賽日報_店點／個人達成率、排名及獎金.xlsx`
     與 `Y26重點台獎手機.xlsx`——**看起來就是台獎的來源**。但 Liam 說先跑通 KPI 再談台獎，
     所以這次沒有碰它們，也沒有做任何台獎欄位推測。

## 2026-07-31 ｜ Claude（第三輪：用真實 0730.xlsx 驗證解析層，抓到並修掉店名比對 BUG）

- 做了什麼：Liam 提供 Drive 的真實 `0730.xlsx`（2.18MB，來源資料夾即
  `KPICALC_SOURCE_FOLDER_ID_DEFAULT`）做解析驗證。**樣本全程只在倉庫外的暫存目錄，
  未進 git、未推 GitHub、未寫任何正式資料、未部署。** 逐項盤點 25 張工作表結構、
  忠實移植 `kpiCalcParseReport` 的 byStore 分支在本機跑一次，再用**真正從 `Code.gs`
  抽出來的驗證函式**跑真實解析結果。
- 結果：解析成功——店點 9 家、人員 40 位、25 項加權欄位齊全、JSON 結構與正式版逐欄一致。
  **但驗證層擋下了它**，因此修了一個 BUG（見下）。修完契約測試 50/50、
  Playwright 28/28、既有 Node 契約 20/20 全綠。
- 經驗 / 給下一位的提醒：
  1. 🚨 **報表店名帶前綴，`STORES` 沒有**：報表是「台北酒泉」「台北通化」，三創更是
     「台灣大哥大數位生活台北三創」，而 `STORES` 常數是「酒泉」「通化」「台北三創」。
     我原本的 `reportUploadValidateKpi_` 用 `STORES.indexOf(name)` 精確比對 →
     **9 家全部落空 → 真實日報被判定「疑似上傳其他區資料」而擋下**。
     已新增 `reportUploadStoreMatch_()` 改雙向包含比對（9/9 命中），
     並加測試確認外區店名仍會被擋。**這種坑只有真實樣本才踩得到。**
  2. **0730.xlsx 沒有 `上線數KPI_個人達成率_明細`**，只有 `_店點` →
     走的是 CLAUDE.md 記載的 byStore 回退分支。**`detail` 主路徑至今仍未用真實檔驗證過。**
  3. **個人表段寬確認不是固定 4 欄**：45 段寬 4、1 段寬 5。
     證實 CLAUDE.md 的警告，`kpiCalcBandsPairs` 逐段偵測是對的，**不可改回 `c += 4`**。
  4. **檔名日期 ≠ 資料日期**：`0730.xlsx` 的期間是 `~ 07/29`，`dataDate = 2026-07-29`。
     版本比較一律以 meta 解析出的日期為準，不要用檔名。
  5. **`kpiCalcParseReport` 不是純函式**：它會 `Drive.Files.create()` 建一張暫存 Google
     試算表再 `setTrashed()` 清掉，另外呼叫 `kpiCalcPrevRoles()` 讀上一份 JSON（唯讀）。
     **不會寫正式 JSON、不寄信、不寫屬性、不發佈**，所以「解析預覽後再確認發佈」成立，
     但要知道它在 Drive 留下短暫的暫存檔。
  6. 其他確認：無隱藏工作表、0 個公式儲存格（不必擔心快取值）、無重複 band 名稱、
     82 個空白列與 9 個「合計」列 parser 均正確略過。
