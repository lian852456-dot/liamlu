...
| repo 程式碼 | ✅ 全部就緒（`gas/Code.gs` **1882 行**，已含所有人的工作＋自動化＋通知信業績重點） |
| patrol.html 改名 | ✅ 已改「督導管理系統」（純前端，不影響待貼的 Code.gs） |
| 同仁看到的資料 | ✅ 已補到累計 **07/27**（07-28 產出，待 Liam 上傳） |
| 日報格式變動 | ⚠️ 0728 起可能不含 `_個人達成率_明細`；解析器已加回退，**Code.gs 因此更新到 1967 行** |
