...
## 2026-07-27 ｜ Claude（正式報銷表列為受控資料源，Y2606 端對端 11 天／74.5 KM 完成）
- 做了什麼：依 Liam 指示，**2026 年 6 月正式公司報銷表本身即為 6/23、6/24、6/30 的正式
  巡店與里程依據**，不再等 `ptread`。`days()` 新增正式受控補充層：`OFFICIAL` 中有、
  但巡店明細沒有的日子，直接採用報銷表的路線與順序；兩店日的當日里程即該唯一一段距離。
  這三筆的 `source` 為「2026 年 6 月正式公司報銷表」、`verify` 為新增的 `official`
  （顯示「人工確認／正式受控」），可計入正式匯出。
  已有巡店明細的日子仍以明細為準，報銷表不覆蓋。`coverage()` 對有 `OFFICIAL` 基準的月份
  不再列缺漏。對帳面板文案改為「已與正式報銷表完成對帳」。
- 結果：**Y2606 端對端 11 個報銷出差日、合計 74.5 KM，逐日 11/11 相符**；
  油料 11 列、距離計算明細 12 段（6/15 拆 4.4＋10.0）、備註全部留白。
  Playwright **58 passed / 2 failed**——兩項失敗（每月班表、半月督導檢查）
  **在 clean main 上同樣失敗**，已用 `git stash` 實測確認，非里程功能造成，未動其他功能。
- 經驗 / 給下一位的提醒：
  * **資料源優先序**：巡店明細 > 正式報銷表補充。報銷表只補明細沒有的日子，不覆蓋既有明細；
    日後 `ptread` 補齊那三天後，會自動改以明細推導，屆時可用來交叉驗證到店時間與順序。
  * 未列於報銷表、也無巡店明細的日期（如 6/25～6/29）**仍不會憑空出現**，已有測試把關。
  * `verify` 四級：`verified`（受控版）／`observed`（歷史觀測）／`manual`（人工確認）／
    `official`（正式受控）。未知路段仍是留白標「待查」，不填 0。
