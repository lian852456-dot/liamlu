...
const state={config:null,draft:null,server:null,submitting:false,mode:'store',ptToken:sessionStorage.getItem(PT_TOKEN_KEY)||'',storeSession:null,uatStoreSelection:false,overview:null,reviewDetail:null,reauthPromise:null,reauthResolve:null,privatePhotoUrls:new Set(),photoViewer:{photos:[],index:0,returnFocus:null}};
...
function isUatBatch(){return /-uat$/i.test(String(state.config?.batch?.batch_id||''));}

function hasRosterProbe(){return Boolean(hasStoreSession()&&isUatBatch()&&state.storeSession.roster_probe_enabled===true);}

function renderRosterProbe(){const card=document.getElementById('rosterProbeCard');if(!card)return;card.hidden=!hasRosterProbe();if(card.hidden){document.getElementById('rosterProbeEmployeeId').value='';const result=document.getElementById('rosterProbeResult');result.textContent='';result.className='inline-message roster-probe-result';}}

function setUatStoreSelection(enabled){const select=document.getElementById('storeSelect');if(!select)return;state.uatStoreSelection=Boolean(enabled&&isUatBatch());select.disabled=!state.uatStoreSelection;select.setAttribute('aria-readonly',state.uatStoreSelection?'false':'true');const first=select.options[0];if(first)first.textContent=state.uatStoreSelection?'請選擇 UAT 店點':'由核准裝置名冊帶入';}

function renderStoreAuth(text){const panel=document.querySelector('.store-auth-panel');const status=document.getElementById('storeAuthMessage');if(!panel||!status)return;const ready=hasStoreSession();panel.classList.toggle('authorized',ready);status.textContent=text||(ready?'本次門市回報已取得短效 audit-only 授權':'尚未取得回報授權');document.getElementById('storeAuthButton').textContent=ready?'重新驗證核准裝置':state.uatStoreSelection?'鎖定 UAT 店點並驗證':'驗證核准裝置';}

function clearStoreSession(text){state.storeSession=null;sessionStorage.removeItem(STORE_SESSION_KEY);setUatStoreSelection(false);renderStoreAuth(text);renderRosterProbe();if(state.config)updateCompletion();}

async function storeAuthorize(){const employeeInput=document.getElementById('storeEmployeeId');const button=document.getElementById('storeAuthButton');const storeSelect=document.getElementById('storeSelect');const employeeId=employeeInput.value.trim().toUpperCase();if(!employeeId){renderStoreAuth('請輸入既有員編');employeeInput.focus();return;}button.disabled=true;renderStoreAuth('正在驗證員編與核准裝置…');try{const result=await api({action:'audit_submit_auth',batch_id:state.config.batch.batch_id,submission_id:state.draft.submission_id,employeeId,deviceId:approvedDeviceId(),uat_store_id:isUatBatch()?(storeSelect.value||state.draft.store_id||''):''});const profile=result.profile||{};if(result.requires_uat_store===true){if(!isUatBatch()||!String(profile.masked_name||'').trim())throw new Error('UAT 授權回應不完整，請聯絡督導');localStorage.setItem(EMPLOYEE_ID_KEY,employeeId);setUatStoreSelection(true);renderStoreAuth(`UAT 督導身分已驗證｜名冊辨識：${profile.masked_name}；請選擇店點後再次驗證`);if(!storeSelect.value)storeSelect.focus();return;}const store=storeById(profile.store_id);if(!result.token||!store||profile.store_name!==store.store_name||!String(profile.masked_name||'').trim())throw new Error('名冊授權回應不完整，請聯絡督導');const hasPhotos=Object.values(state.draft.items||{}).some(item=>(item.photos||[]).some(photo=>!photo.deleted));if(state.draft.store_id&&state.draft.store_id!==profile.store_id&&hasPhotos)throw new Error('本機草稿屬於不同店點，照片已保留；請使用原核准裝置或請督導取消重設');state.draft.store_id=profile.store_id;storeSelect.value=profile.store_id;setUatStoreSelection(false);saveDraft();state.storeSession={token:result.token,auth_source:'approved-device',batch_id:state.config.batch.batch_id,store_id:profile.store_id,submission_id:state.draft.submission_id,roster_probe_enabled:profile.roster_probe_enabled===true};localStorage.setItem(EMPLOYEE_ID_KEY,employeeId);sessionStorage.setItem(STORE_SESSION_KEY,JSON.stringify(state.storeSession));renderStoreAuth(`核准裝置驗證成功：${store.store_name}｜名冊辨識：${profile.masked_name}`);renderRosterProbe();updateCompletion();await restoreOwnSubmission();}catch(error){clearStoreSession(error.unauthorized?'此員編尚未核准此裝置，請先使用既有申請流程':error.message);}finally{button.disabled=false;}}

async function probeRoster(){const input=document.getElementById('rosterProbeEmployeeId');const button=document.getElementById('rosterProbeButton');const result=document.getElementById('rosterProbeResult');const employeeId=input.value.trim().toUpperCase();result.className='inline-message roster-probe-result';if(!employeeId){result.textContent='請輸入欲核對的同仁員編';input.focus();return;}button.disabled=true;result.textContent='正在唯讀核對名冊…';try{const profile=await storeCall({action:'audit_roster_probe',submission_id:state.draft.submission_id,roster_employee_id:employeeId});if(!profile.exists){result.textContent='名冊查無此員編';result.classList.add('error');return;}const status=profile.roster_status==='active'?'啟用':'停用';const device=profile.approved_device_bound?'已綁定 Approved Device':'尚未綁定 Approved Device';if(profile.store_mapping_ok){result.textContent=`名冊辨識：${profile.masked_name||'—'}｜${profile.roster_store||'—'}｜狀態：${status}｜店點映射：PASS（${profile.audit_store_name}）｜${device}`;}else{result.textContent=`名冊辨識：${profile.masked_name||'—'}｜狀態：${status}｜店點映射失敗：名冊值 ${profile.roster_store||'（空白）'} 無法對應北一二B九店｜${device}`;result.classList.add('error');}}catch(error){result.textContent=error.message;result.classList.add('error');}finally{button.disabled=false;}}
...
function renderConfig(){
  ...
  document.getElementById('storeEmployeeId').value=localStorage.getItem(EMPLOYEE_ID_KEY)||'';
  renderStoreAuth();renderRosterProbe();renderItems();saveDraft();
}
...
async function hydratePhotoUrls(){
  for(const item of state.config.items){for(const photo of itemState(item.item_id).photos){if(!photo.objectUrl&&!photo.deleted){const stored=await dbGet(blobKey(photo.id));if(stored?.bytes) photo.objectUrl=URL.createObjectURL(new Blob([stored.bytes],{type:stored.type||photo.type||'image/jpeg'}));}}}
}
...
async function removePhoto(itemId,photoId){
  ...
  if(photo.status==='uploaded'){
    try{await storeCall({action:'audit_photo_delete',submission_id:state.draft.submission_id,edit_token:state.draft.edit_token,client_photo_id:photo.id});}
    catch(error){message(error.message,'error');return;}
  }
  if(photo.objectUrl){URL.revokeObjectURL(photo.objectUrl);state.privatePhotoUrls.delete(photo.objectUrl);photo.objectUrl='';}photo.deleted=true;await dbDelete(blobKey(photo.id));saveDraft();renderItemPhotos(itemId);updateCompletion();
}

function itemPhotos(itemId){return itemState(itemId).photos.filter(photo=>!photo.deleted);}

function privatePhotoObjectUrl(mimeType,base64){if(!/^image\//.test(String(mimeType||'')))throw new Error('照片格式不正確');const binary=atob(String(base64||''));const bytes=new Uint8Array(binary.length);for(let index=0;index<binary.length;index++)bytes[index]=binary.charCodeAt(index);const url=URL.createObjectURL(new Blob([bytes],{type:mimeType}));state.privatePhotoUrls.add(url);return url;}

async function ensurePrivatePhoto(photo,scope,submissionId){if(photo.objectUrl)return photo.objectUrl;if(photo.readPromise)return await photo.readPromise;const meta=photo.server||photo;if(!meta?.client_photo_id)throw new Error('照片識別資料不完整');const request={action:'audit_photo_read',submission_id:submissionId,client_photo_id:meta.client_photo_id};photo.readPromise=(async()=>{const result=await(scope==='supervisor'?supervisorCall(request):storeCall({...request,edit_token:state.draft.edit_token}));photo.objectUrl=privatePhotoObjectUrl(result.mime_type,result.base64);photo.privateObjectUrl=true;return photo.objectUrl;})();try{return await photo.readPromise;}finally{photo.readPromise=null;}}

function revokePrivatePhotos(photos){(photos||[]).forEach(photo=>{if(photo.privateObjectUrl&&photo.objectUrl){URL.revokeObjectURL(photo.objectUrl);state.privatePhotoUrls.delete(photo.objectUrl);photo.objectUrl='';photo.privateObjectUrl=false;}});}

function revokeDraftPhotos(photos){(photos||[]).forEach(photo=>{if(photo.objectUrl){URL.revokeObjectURL(photo.objectUrl);state.privatePhotoUrls.delete(photo.objectUrl);photo.objectUrl='';photo.privateObjectUrl=false;}});}

function loadTilePhoto(tile,img,photo,scope,submissionId){const loading=document.createElement('span');loading.className='photo-loading';loading.textContent='照片載入中…';tile.insertBefore(loading,tile.firstChild);ensurePrivatePhoto(photo,scope,submissionId).then(url=>{if(!tile.isConnected)return;img.src=url;loading.replaceWith(img);}).catch(error=>{if(tile.isConnected)loading.textContent=error.message||'照片載入失敗';});}
...
function renderItemPhotos(itemId){
  ...
  photos.forEach((photo,index)=>{
    const tile=document.createElement('div');tile.className='photo-tile';const img=document.createElement('img');img.alt=`第 ${index+1} 張照片`;if(photo.objectUrl){img.src=photo.objectUrl;tile.appendChild(img);}else if(photo.server)loadTilePhoto(tile,img,photo,'store',state.draft.submission_id);else tile.appendChild(img);
    const preview=document.createElement('button');preview.type='button';preview.className='preview-button';preview.setAttribute('aria-label',`放大預覽第 ${index+1} 張照片`);preview.addEventListener('click',async()=>{try{await Promise.all(photos.filter(row=>row.server&&!row.objectUrl).map(row=>ensurePrivatePhoto(row,'store',state.draft.submission_id)));openPhotoViewer(photos,index,preview);}catch(error){message(error.message,'error');}});tile.appendChild(preview);
...
function validation(){
  ...
  if(!hasStoreSession())errors.push('門市回報授權');
...
async function ensureSubmission(){
  return storeCall({action:'audit_start',batch_id:state.config.batch.batch_id,submission_id:state.draft.submission_id,edit_token:state.draft.edit_token,inspector_name:state.draft.inspector_name});
}
...
async function uploadPending(){
  ...
  for(let index=0;index<targets.length;index++){
    ...
    try{
      ...
      const result=await storeCall({action:'audit_upload',submission_id:state.draft.submission_id,edit_token:state.draft.edit_token,item_id:itemId,client_photo_id:photo.id,note:state.draft.notes[itemId]||'',file:{name:photo.name,type:compressed.type||photo.type||'image/jpeg',size:compressed.size,base64}});
...
async function submitReport(event){
  ...
  try{await ensureSubmission();await uploadPending();message('照片已上傳，正在寫入回報並讀回確認…');const result=await storeCall({action:'audit_submit',submission_id:state.draft.submission_id,edit_token:state.draft.edit_token,notes:state.draft.notes});if(result.readback_verified!==true)throw new Error('雲端讀回尚未確認，請稍後重試');applyServerStatus(result);message('全部寫入並讀回確認完成。','success');}
...
function renderServerState(){
  ...
  const closed=['submitted','approved','cancelled'].includes(server.submission_status);document.getElementById('auditForm').hidden=closed;document.getElementById('completionCard').hidden=!closed&&server.submission_status!=='rework';
...
function renderReviewDetail(detail,focusItemId){
  if(state.reviewDetail)revokePrivatePhotos(state.reviewDetail.items.flatMap(item=>item.photos||[]));state.reviewDetail=detail;const cancelled=detail.submission_status==='cancelled';
  document.getElementById('reviewDetail').innerHTML=`<h2>${escapeHtml(detail.store_name)}｜督導驗收</h2><p>檢查人員：${escapeHtml(detail.inspector_name)}｜首次回報：${escapeHtml(detail.submitted_at||'—')}｜最後更新：${escapeHtml(detail.updated_at||'—')}</p>`+detail.items.map(item=>`<section class="review-section" data-review-item="${escapeHtml(item.item_id)}"><h3>${escapeHtml(item.item_name)} <span class="status-chip ${escapeHtml(item.status)}">${escapeHtml(STATUS_LABELS[item.status]||item.status)}</span></h3><p>備註：${escapeHtml(item.note||'—')}</p><div class="photo-grid">${(item.photos||[]).map((photo,index)=>`<button type="button" class="photo-tile supervisor-photo" data-item-id="${escapeHtml(item.item_id)}" data-photo-index="${index}" aria-label="放大查看${escapeHtml(item.item_name)}第 ${index+1} 張"><span class="photo-loading">照片載入中…</span><span class="photo-state uploaded">revision ${photo.revision}</span></button>`).join('')}</div>${item.reviewer_comment?`<p class="return-reason">退回原因：${escapeHtml(item.reviewer_comment)}</p>`:''}${cancelled?'':`<label class="review-comment"><span>退回原因</span><textarea rows="2" maxlength="300" placeholder="選擇退回補件時必填"></textarea></label><div class="review-actions"><button type="button" class="approve-button" data-decision="approve">通過</button><button type="button" class="return-button" data-decision="return">退回補件</button></div>`}</section>`).join('')+`<div class="timeline"><h3>時間軸</h3>${(detail.timeline||[]).map(event=>`<div class="timeline-entry">${escapeHtml(event.created_at)}｜${escapeHtml(event.item_name||'整筆')}｜${escapeHtml(event.event_type)}${event.comment?`｜${escapeHtml(event.comment)}`:''}</div>`).join('')}</div>${cancelled?'<p class="message error">此回報已取消；原照片與事件紀錄仍保留。</p>':'<button type="button" class="secondary-button cancel-submission-button">取消／重設此門市回報</button>'}`;
  document.querySelectorAll('.supervisor-photo').forEach(button=>{const item=detail.items.find(row=>row.item_id===button.dataset.itemId);const photo=item.photos[Number(button.dataset.photoIndex)];const loading=button.querySelector('.photo-loading');ensurePrivatePhoto(photo,'supervisor',detail.submission_id).then(url=>{if(!button.isConnected)return;const img=document.createElement('img');img.src=url;img.alt=`${item.item_name}第 ${Number(button.dataset.photoIndex)+1} 張`;loading.replaceWith(img);}).catch(error=>{if(button.isConnected)loading.textContent=error.message||'照片載入失敗';});button.addEventListener('click',async()=>{try{await Promise.all(item.photos.map(row=>ensurePrivatePhoto(row,'supervisor',detail.submission_id)));openPhotoViewer(item.photos,Number(button.dataset.photoIndex),button);}catch(error){setSupervisorActionMessage(error.message,'error');}});});
...
function setSupervisorActionMessage(text,type='success'){const box=document.getElementById('supervisorActionMessage');box.hidden=!text;box.textContent=text||'';box.className=`message ${type}`.trim();}

async function cancelSubmission(detail){if(!window.confirm(`確認取消 ${detail.store_name} 本次回報？\n\n照片與事件證據會保留，門市將可建立新的回報。`))return;const button=document.querySelector('.cancel-submission-button');if(button)button.disabled=true;try{const cancelled=await supervisorCall({action:'audit_cancel',submission_id:detail.submission_id,comment:'督導取消並開放門市重新回報'});renderReviewDetail(cancelled);await loadOverview();setSupervisorActionMessage(`${detail.store_name} 舊回報已取消並保留證據，門市現在可重新回報。`);}catch(error){setSupervisorActionMessage(error.message,'error');if(button)button.disabled=false;}}
...
function bindEvents(){
  ...
  document.getElementById('storeAuthButton').addEventListener('click',storeAuthorize);document.getElementById('storeEmployeeId').addEventListener('keydown',event=>{if(event.key==='Enter'){event.preventDefault();storeAuthorize();}});document.getElementById('rosterProbeButton').addEventListener('click',probeRoster);document.getElementById('rosterProbeEmployeeId').addEventListener('keydown',event=>{if(event.key==='Enter'){event.preventDefault();probeRoster();}});document.getElementById('storeSelect').addEventListener('change',event=>{if(!state.uatStoreSelection)return;state.draft.store_id=event.target.value;saveDraft();updateCompletion();});document.getElementById('inspectorName').addEventListener('input',event=>{state.draft.inspector_name=event.target.value;saveDraft();updateCompletion();});document.getElementById('newSubmissionButton').addEventListener('click',resetCancelledSubmission);
...
async function boot(){
  loadDraft();loadStoreSession();bindEvents();
  try{state.config=await api({action:'audit_config'});if(state.config.contract!=='audit-cleaning-v1')throw new Error('稽核服務版本不相容');if(state.draft.batch_id&&state.draft.batch_id!==state.config.batch.batch_id){clearStoreSession();state.draft=blankDraft();}state.draft.batch_id=state.config.batch.batch_id;renderConfig();await hydratePhotoUrls();renderItems();await restoreOwnSubmission();}
...
window.addEventListener('pagehide',releasePrivatePhotoUrls);window.addEventListener('beforeunload',releasePrivatePhotoUrls);
