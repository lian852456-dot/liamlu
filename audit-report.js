...
function ensurePrivatePhoto(photo,scope,submissionId){if(photo.objectUrl)return Promise.resolve(photo.objectUrl);if(photo.readPromise)return photo.readPromise;const meta=photo.server||photo;if(!meta?.client_photo_id)return Promise.reject(new Error('照片識別資料不完整'));const request={action:'audit_photo_read',submission_id:submissionId,client_photo_id:meta.client_photo_id};photo.readPromise=(scope==='supervisor'?supervisorCall(request):storeCall({...request,edit_token:state.draft.edit_token})).then(result=>{photo.objectUrl=privatePhotoObjectUrl(result.mime_type,result.base64);photo.privateObjectUrl=true;return photo.objectUrl;}).finally(()=>{photo.readPromise=null;});return photo.readPromise;}
...
window.addEventListener('beforeunload',()=>{state.privatePhotoUrls.forEach(url=>URL.revokeObjectURL(url));state.privatePhotoUrls.clear();});
