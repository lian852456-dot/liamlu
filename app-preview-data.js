(function attachPreviewData(scope) {
  ...
  const previewPeople=stores.slice(0,9).map((store,index)=>({
    ...
    totalRate:index%3===0?0:Number((1.12-index*.045).toFixed(3)),rank:index%3===0?1326:12+index*7,dod:index%3===0?0:Number((.028-index*.009).toFixed(3)),rankChange:index%3===0?0:2-index,
