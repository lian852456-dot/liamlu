...
另有 `kpi.html`（KPI 試算網站，2026-07 新增）：純前端單檔、無後端，內建公司日報
（上線數KPI 店點/個人明細）的目標數、累計實際數與權重，同仁 KEY 今日上線數即可
試算各項目與「明日 KPI 總進度達成率」。計分公式由「KPIPI資料設定」模板＋0720 日報
反推驗證（逐項 100% 吻合、總分 7/9 店完全一致，殘差由校正值吸收），細節見
`docs/COLLAB-LOG.md` 2026-07-20 該則。日報數據更新時需重新產生內嵌 JSON（或用
頁內「修改累計」手動更新）；localStorage 鍵：`bei12b_kpi_v1`。
