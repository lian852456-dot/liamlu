...
- 這個開發環境的 proxy **封鎖 script.google.com**（403），無法直接 curl/fetch 測 GAS。
  **查資料**請改走 Google Drive 連接器直接讀試算表（見下方「驗證方式」節）；
  **驗部署版本**仍只能請 Liam 開 `?action=ping` / `?action=read&date=...&seg=16` / `?action=debug`
...
2026-07-20 起，Claude 的遠端工作環境已接上 Liam 的 Google Drive 連接器，
可直接讀取試算表內容（「回報資料」「巡店明細」等工作表都讀得到）。
驗證分三層，**各層能證明的事不同，試算表有資料 ≠ GAS 部署正常 ≠ 正式網站正常**：

### 第 1 層：試算表資料驗證 → AI 直接讀，不要再請 Liam 複製 JSON
凡是「資料是否寫入」「欄位（標題列）是否存在」「歷史資料是否完整」這類問題，
一律優先透過 Drive 連接器直接讀試算表確認（ID 見「架構」節）。
**不要再請 Liam 開 `?action=read` / `?action=debug` 人工複製回傳結果。**
（Codex 等沒有 Drive 連接器的協作環境不適用本層，照舊流程請 Liam 幫忙。）

### 第 2 層：GAS 部署版本驗證 → 讀試算表看不出來，需 Liam 開瀏覽器
試算表有資料只代表「某個版本的 GAS」寫過資料，不代表**編輯器裡的最新程式碼已部署生效**。
改了 `doGet` 相關程式碼後，仍需 Liam 開 `{GAS_URL}?action=debug` 確認
（回 `unknown action` = 舊版還在跑）。純時間觸發器改動（信件內容等）只需存檔，
不用重新部署，見「自動檢查未回報」節。

### 第 3 層：正式網站端到端操作驗證 → 需 Liam 實機操作
GitHub Pages 正式站上「實際填報 → 送出 → 彙整顯示」的完整流程，只有 Liam
在真實裝置操作過才算數。本地 Playwright 測試可攔截 API 模擬流程，
但不能替代正式站驗證。

### 需要 Liam 人工驗證的部分：集中成一次性驗收清單
第 2、3 層需要人工的項目，完工時**整理成一份驗收清單一次交付**
（列清楚：要開哪些網址、操作哪些步驟、預期看到什麼），不要做一步問一步、逐項來回。
...
3. 重新部署 GAS（新版本！）
4. 欄位確認：透過 Drive 連接器直接讀試算表標題列（無連接器的環境才用 `?action=debug`）
5. 填一筆測試資料 → 直接讀試算表確認該筆存在；「部署版本驗證（`?action=debug`）」
   與「正式站實測」列入給 Liam 的一次性驗收清單（見「驗證方式」節）
