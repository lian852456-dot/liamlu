...
- `app.html` 自 2026/09 起與 `patrol.html` 共用 `patrol-question-versions.js`：App 以正式 `ptdetail` 呈現新版 25 項、每店每月 2 次且至少相隔 7 天；並以既有唯讀 `ptmileage2` 顯示本月累積公里、報銷出差日、待確認路段與最近路線。8 月以前的 33 題歷史摘要維持原規則。
- App 的「督導到店檢查」同樣依日期切版：2026/09/01 起顯示第 1–9 項，舊日期保留 18 題。上述 App 增補均沿用既有巡店短效 session，沒有新增正式寫入 action。
...
- `home.html` 的督導專區提供 `phone-stock-dashboard.html`；使用者上傳銷售與庫存檔後，瀏覽器在本機解析北一二B九店資料。按「同步庫存到 APP」才會將機款、店點、庫存數送至既有私有 GAS；APP「戰情 → 庫存」以同一套核准裝置識別讀取。
- 銷售檔支援「店點、機型、銷售日期、銷售數」直列，或將各日期做成欄位的近三日分欄格式；庫存檔需「店點、機型、庫存數」，如有庫存日期會選資料截止日前最新快照。
- 去化率固定為「近三日銷售 ÷（近三日銷售＋目前庫存）」；庫存同步不包含銷售數，也不保存原始檔。網站與 APP 均需該督導員編綁定已核准裝置；前端部署後仍須更新對應的私有 GAS Web App 部署才可跨裝置同步。
...
- 正式目標是選用加值：沿用 Approved Device 的 `private_access → kpicalc_access` 唯讀流程並要求 `profile.isTrusted=true`，以各項「月目標－正式截止累積實績」除以截止後剩餘天數、無條件進位，追加今日動態目標與目前尚缺。較昨日舊但仍屬當月的正式資料會標示落後天數後繼續計算；未來日期、跨月或五項資料不完整仍會停止冒算。
