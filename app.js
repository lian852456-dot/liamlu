(function startLiamSupervisorApp(scope) {
  ...
  function halfMonthStatus(store) {
    ...
    if(store.answeredItems<18) return [`${store.answeredItems}/18 已填`,'gold-value','loader-circle'];
    return ['18/18 已填',store.abnormalCount?'gold-value':'positive',store.abnormalCount?'triangle-alert':'check-circle-2'];
  }
  ...
  function renderHalfMonthOverview(data) {
    ...
    const readBanner=PREVIEW_MODE?'<b>PREVIEW / 尚未寫入正式資料</b><span>本頁只驗證資訊架構；不會寫入正式資料或上傳媒體。</span>':'<b>FORMAL READ / 正式唯讀</b><span>資料來自 hread；填寫進度是 18 題完整度，不是 backend completed。</span>';
    return `<section class="preview-only-banner">${readBanner}</section>
      ...
      <section class="half-preview-summary" aria-label="本期半月檢查摘要">
        <article><span>18/18 已填</span><b>${summary.filledStores==null?'—':`${summary.filledStores} / ${summary.totalStores}`}</b></article>
      ...
      <button class="half-preview-start" type="button" data-half-preview-action="start">${PREVIEW_MODE?'+ 開始檢查 Preview':'查看半月檢查內容'}</button>`;
  }
  ...
  function renderHalfMonthCheck() {
    ...
    if(!PREVIEW_MODE&&halfMonthReadState==='loading'){
      container.innerHTML='<section class="preview-only-banner"><b>FORMAL READ / 唯讀</b><span>正在透過既有短效 session 讀取 hread；不會呼叫正式 write。</span></section><div class="empty-state">正在讀取半月督導檢查…</div>';
      return;
    }
    ...
    if(!data){
      container.innerHTML='<section class="preview-only-banner"><b>FORMAL READ / 唯讀</b><span>切換至本頁後才讀取 hread；不會呼叫正式 write。</span></section><div class="empty-state">準備讀取半月督導檢查。</div>';
      return;
    }
  ...
  async function loadHalfMonthFormalRead() {
    ...
    try{
      ...
      halfMonthReadMessage=String(error&&error.message||'正式半月資料讀取失敗。');
  ...
  async function loadPatrolData() {
    ...
    const patrolTask=patrolRead('ptsummary',{month}).then(result=>{
      const readAt=nowIso();
      patrolRaw=result; const data=adaptPatrolSummary(patrolRaw,month); const sourceUpdatedAt=data.sourceUpdatedAt||readAt;
      contract.patrolOverview=C.moduleState({status:'ok',updatedAt:readAt,sourceUpdatedAt,stale:false,source:moduleSource('巡店 ptsummary','patrol.html'),data});
      contract.patrolStores=C.moduleState({status:'ok',updatedAt:readAt,sourceUpdatedAt,stale:false,source:moduleSource('巡店 ptsummary','patrol.html'),data:data.stores});
      contract.patrolToday=C.moduleState({status:'no_data',updatedAt:readAt,sourceUpdatedAt,stale:false,source:moduleSource('巡店 ptsummary','patrol.html'),data:null,note:'現有正式來源未提供今日預定路線與移動時間'});
      contract.generatedAt=readAt;
      renderAll();
    }).catch(error=>{
      const expired=/授權已逾時/.test(error.message);
      const note=String(error.message||readErrorNote(error,'正式巡店'));
      if(!expired&&previousPatrolOverview.data){ contract.patrolOverview=C.moduleState({...previousPatrolOverview,status:'stale',stale:true,note:`上次成功資料 · ${formatTime(previousPatrolOverview.updatedAt)} · ${note}`}); contract.patrolStores=C.moduleState({...previousPatrolStores,status:'stale',stale:true,note:`上次成功資料 · ${formatTime(previousPatrolStores.updatedAt)} · ${note}`}); contract.patrolToday=C.moduleState({...previousPatrolToday,status:'stale',stale:Boolean(previousPatrolToday.data),note:`上次成功資料 · ${formatTime(previousPatrolToday.updatedAt)} · ${note}`}); }
    ...
    return Promise.allSettled([scheduleTask,patrolTask,visitTask]);
  }
  ...
  if ('serviceWorker' in navigator && location.protocol !== 'file:') scope.addEventListener('load',()=>navigator.serviceWorker.register('./service-worker.js?v=pwa-iphone-stable-20260824-1',{scope:'./',updateViaCache:'none'}).catch(()=>{}));
})(window);
